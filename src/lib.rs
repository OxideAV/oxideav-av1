...
pub use cdf::{
    ...
    InterFrameContext, IntraFrameModeInfoPrefix, PaletteColorContext, PalettePlane,
    PaletteTokensArgs, PartitionWalker, TileCdfContext, TileGeometry, ADJUSTED_TX_SIZE, ADST_ADST,
    ADST_DCT, ADST_FLIPADST, BLOCK_128X128, BLOCK_128X64, BLOCK_16X16, BLOCK_16X32, BLOCK_16X4,
    BLOCK_16X64, BLOCK_16X8, BLOCK_32X16, BLOCK_32X32, BLOCK_32X64, BLOCK_32X8, BLOCK_4X16,
    BLOCK_4X4, BLOCK_4X8, BLOCK_64X128, BLOCK_64X16, BLOCK_64X32, BLOCK_64X64, BLOCK_8X16,
    BLOCK_8X32, BLOCK_8X4, BLOCK_8X8, BLOCK_INVALID, BLOCK_SIZES, BLOCK_SIZE_GROUPS, BR_CDF_SIZE,
    BWD_REFS, CFL_ALPHABET_SIZE, CFL_ALPHA_CONTEXTS, CFL_JOINT_SIGNS, CLASS0_SIZE,
    COEFF_BASE_CTX_OFFSET, COEFF_BASE_POS_CTX_OFFSET, COEFF_BASE_RANGE, COEFF_CDF_Q_CTXS,
    COMPOUND_IDX_CONTEXTS, COMPOUND_MODES, COMPOUND_MODE_CONTEXTS, COMPOUND_MODE_CTX_MAP,
    COMPOUND_TYPES, COMP_GROUP_IDX_CONTEXTS, COMP_INTER_CONTEXTS, COMP_NEWMV_CTXS,
    COMP_REF_TYPE_CONTEXTS, DCT_ADST, DCT_DCT, DCT_FLIPADST, DC_SIGN_CONTEXTS,
    DEFAULT_ANGLE_DELTA_CDF, DEFAULT_CFL_ALPHA_CDF, DEFAULT_CFL_SIGN_CDF, DEFAULT_COEFF_BASE_CDF,
    ...
    INTER_TX_TYPE_SET1_SIZES, INTER_TX_TYPE_SET3_SIZES, INTRABC_DELAY_PIXELS, INTRA_FILTER_MODES,
    INTRA_MODES, INTRA_MODE_CONTEXT, INTRA_MODE_CONTEXTS, INTRA_TX_TYPE_SET1_SIZES,
    INTRA_TX_TYPE_SET2_SIZES, IS_INTER_CONTEXTS, LEVEL_CONTEXTS, MAG_REF_OFFSET_WITH_TX_CLASS,
    MAX_ANGLE_DELTA, MAX_TX_DEPTH, MAX_TX_DEPTH_TABLE, MAX_TX_SIZE_RECT, MAX_VARTX_DEPTH,
...
// r186: §7.11.2.4 step-10 V_PRED + step-11 H_PRED sample-generation
// leaves admitted to the dispatcher alongside DC_PRED; plus the
// `DC_PRED` / `H_PRED` named §6.10.x intra-mode ordinals (`V_PRED`
...
pub use inter_pred::{
    ...
    reconstruct_inter_block, reconstruct_inter_block_interintra, reconstruct_inter_frame,
    reconstruct_inter_intra_block, reconstruct_inter_intra_from_dispatch, resolve_divisor,
    rounding_variables, select_interp_filter_small_block, setup_shear, warp_estimation, wedge_mask,
    CompoundParams, DistanceWeights, Divisor, InterIntraLeaf, InterIntraModeInfo,
    InterIntraWedgeModeInfo, InterModeInfo, InterModeInfoGrid, LocalWarp, MvScale,
    PlaneReconContext, PredictInterRef, RefFrameStoreEntry, RoundingVars, ShearParams,
    WarpDriverParams, WarpSampleCand, COMPOUND_AVERAGE, COMPOUND_DIFFWTD, COMPOUND_DISTANCE,
    COMPOUND_INTRA, COMPOUND_WEDGE, DIV_LUT, DIV_LUT_BITS, DIV_LUT_NUM, DIV_LUT_PREC_BITS,
    EIGHTTAP_4TAP, EIGHTTAP_SMOOTH_4TAP, FILTER_BITS as INTER_FILTER_BITS, II_DC_PRED, II_H_PRED,
    II_SMOOTH_PRED, II_V_PRED, II_WEIGHTS_1D,
...
pub enum Error {
    ...
    /// [`crate::rounding_variables`] for §7.11.3.2). Per r191 the
    /// §7.11.3.11-15 compound-mask / blend bodies landed; per r192 the
    /// §7.11.3.5-8 WARP MC kernel landed via [`crate::block_warp`] /
    /// [`crate::setup_shear`] / [`crate::resolve_divisor`] /
    /// [`crate::warp_estimation`]. The remaining §7.11.3.9-10 OBMC
    /// bodies are next-arc.
    ///
    ...
    /// `intra_frame_y_mode` reader's reachable cell), so the
    /// §5.11.5-driven path never fires this.
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::ComputePredictionInterUnsupported => write!(
                f,
                "oxideav-av1: §5.11.33 compute_prediction reached §7.11.3 predict_inter — defensive fallback retained post-r192 (§7.11.3.4 translational MC kernel via inter_pred::block_inter_prediction; §7.11.3.11-15 compound bodies (wedge / diffwtd / intra / mask_blend / distance) per r191; §7.11.3.5-8 WARP MC via block_warp / setup_shear / resolve_divisor / warp_estimation per r192; §7.11.3.9-10 OBMC still pending next-arc, plus the predict_inter driver wiring)"
            ),
