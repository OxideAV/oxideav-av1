...
- §7.17 RESTORATION UNIT-SIZE LADDER (r460): the KEY / still election runs the per-unit search at 64, 128 and 256-sample units (`LrElectInput::unit_shift`, the §5.9.20 `lr_unit_shift`; 128 / 256 only under 128×128 superblocks) — each a complete election + tile re-emission — and keeps the unit size whose exact `D·256 + λ·bits` wins against the unfiltered frame. Larger units amortise the per-unit Wiener / SGR signalling (43 bits per Wiener unit), which is what lets restoration pay on the pictures where 64-sample units never did. Each rung is a complete election, so the presets bound the ladder (`SearchLimits::lr_min_unit_shift`: `Fast` 256-sample units only, `Balanced` 128 / 256, `Thorough` all three) — at 12 MP the three-rung ladder was 55 % of the still's wall clock (8-thread `Fast` 95 s → 37 s with the single rung). En route the `lr_ab` / `seg_pairings` / `superres_ab` / `mixed_lossless` / `film_grain_lag3` / `cdef_unit_ab` election-premise witnesses were re-tuned: with the forward-transform gain corrected, the reconstruction no longer carries the structured error those elections used to clean at the old quantisers / frame sizes (documented per witness in the test bodies — every stream still decodes bit-exact); `film_grain_seg_extras` holds the §5.9.17 delta-q arm off (a P-frame-driver reconstruction divergence of ±1 at one chroma block edge under delta-q × feature-extra tables × the grain re-encode — confirmed against three independent black-box decoders, tracked as a followup)

- TILE-PARALLEL STILL SEARCH (r460): the per-tile RD walk is one closure run sequentially or on `SearchLimits::threads` scoped threads, each on a private copy of the reconstruction / syntax mirror, stitched back in tile order (`snapshot_price_scope` over the tile rect) — bit-identical to the sequential walk (witness `threaded_tile_search_is_bit_identical`). `StillOptions::threads(n)` / the encoder option `threads` derive a tile layout from the picture size (`auto_tile_layout`: tiles ≥ 256 px wide / 128 px tall, at most `n.next_power_of_two()`; explicit `tile_*_log2` win). A requested layout below the §5.9.15 floor is raised to the legal minimum (`TileInfo::min_uniform_layout` — a 12 MP picture cannot be one tile; the pre-r460 KEY entries refused it)

- CfL LEAST-SQUARES ALPHA (r460): the chroma mode pick seeds `(αU, αV)` from the per-plane least-squares fit `64 · Σ(res · ac) / Σ(ac²)` over the §7.11.5 luma AC term and trials each seed ±1 (nine pairs) instead of the fixed 20-pair grid that reached no further than |α| = 4

- FORWARD-TRANSFORM GAIN — the intra encoder's headline quality defect (r460). Probed against the decoder's own §7.13.3 chain (forward → quantise at a fine step → §7.12.3 dequantise → inverse), the column-then-row forward composition reproduced the residual at 1/4 amplitude for the 4×4 family, 1/2 for 8×8 / 8×16 / 4×16, exactly 1 only at 16×16 / 8×32, 2 at 16×32 and 4 at 32×32 — every transform-unit size but 16×16 coded a residual the decoder rebuilds at the wrong amplitude, and since the search measured its true (mis-scaled) distortion the streams stayed conformant while the RD ladders compensated around the defect (unit tests had enshrined the empirical {1/4, 1/2, 1, 4, 4} scales as the contract). `forward_transform_2d` now applies the per-size correction (`2^(4 - floor((log2W + log2H) / 2))` up to a 16×16-sized product, `2^(8 - log2W - log2H)` beyond) so `inverse(quantise(forward(r))) ≈ r` at every size and every DCT / ADST / identity pairing (rms error ≤ 0.5 on ±40 residuals; the 64-point family keeps the spec's own 32×32 coefficient retention). Measured on a 640×480 textured still, third-party encoder at its default speed as the reference (Bjontegaard, PSNR): +70.7 % → **+17.1 % BD-rate** for the `Fast` still preset; at `base_q_idx = 100`: 8.2 KB / 33.3 dB → 16.5 KB / 42.9 dB (the reference: 15.0 KB / 43.6 dB at the same qindex). The transform unit tests assert the unit-gain contract; the r425 rect-palette witness pins content where the residual is not free (a legitimate RD change)

- FLIPADST FAMILY in the luma transform-type search (r460): the §5.11.47 candidate distortion and the committed reconstruction now read the inverse-transformed residual through the §7.12.3 step-3 `flipUD` / `flipLR` destination remap (`step3_flips`) — the pre-r460 search scored the eight FLIPADST types against the unflipped residual, so they never won; the committed leaf would have mismatched the decoder had one been chosen

- STILL SPEED PRESETS — `SearchLimits` (r460): the still-picture `speed` dial now tightens the intra RD ladders (`Fast`: one `Split_Tx_Size` step, the §5.11.47 shortlist `DCT_DCT` + `Mode_To_Txfm` default (+ IDTX under screen-content tools), no rect partition arms, no 4×4 leaves, residual-free square leaves end the descent, every fourth §7.17 self-guided set, coarse deblock ladder; `Balanced`: full TX ladder + rect arms + 4×4 leaves with the shortlist and the early exit, every second SGR set, refined deblock ladder; `Thorough`: the exhaustive pre-r460 search + the QM / superres elections). Default `SearchLimits` keep every other entry point bit-identical. 640×480 textured still: 11.8 s → 3.1 s (`Fast`); a 4032×3024 8-bit still at `base_q_idx = 128`: 46 s single-thread / 37 s on 8 threads (`Fast`, 116.8 KB / 41.8 dB luma PSNR; the third-party encoder at its default speed: 0.5 s, 147.8 KB / 47.3 dB at the same qindex — the frame-level CDEF / restoration / deblock elections, not the tile search, now dominate the 12 MP wall clock), 10-bit 59 s / 51 s. The pre-r460 KEY path could not encode a 12 MP picture at all (one-tile layouts above `MAX_TILE_AREA` were refused; extrapolated from its 640×480 wall clock it would have taken ~55 min). Also: a luma colour-count probe gates `allow_screen_content_tools` on the presets (photographic content skips the palette / intra-bc searches and the per-block palette syntax); the QM election is `Thorough`-only for stills (it trades PSNR for bytes)

- §7.14 DEBLOCKING-LEVEL ELECTION (r460, `encoder::lf_elect`): every intra frame this crate coded before r460 carried `loop_filter_level = 0` (no deblocking at all). The election runs the decoder's own filter (the write-side mirror's `loop_filter_frame_from_grid`) on the reconstruction at a coarse-to-fine ladder of levels and keeps the lowest-SSD luma level then chroma level (`loop_filter_delta_enabled = 0`, sharpness 0); it runs first in the §7.4 in-loop order, ahead of the CDEF / LR elections. On by the still presets (`SearchLimits::lf_effort`), off by default elsewhere

- DEAD-ZONE QUANTISER ROUNDING (r460): `forward_quantize_rounded(.., round_num_64)` — `|level| = floor(|c| / step + round / 64)` with the DC coefficient always rounding to nearest; the still presets quantise AC at `22 / 64`, every other entry keeps the round-to-nearest `32 / 64`

- ANY PICTURE EXTENT on the KEY / still path (r460): `encode_key_frame_yuv_full` pads a non-multiple-of-8 picture to the §5.9.5 mi grid (`YuvFrame::padded_to_mi_grid`, edge replication) and codes it under the TRUE extent (`KeyExtras::coded_size` → sequence maximum, §5.9.5 `frame_size`, the §7.17 restoration unit / stripe geometry via the new `LrElectInput::frame_width/height` + `apply_lr_plan` extent args, IVF header), cropping the returned reconstruction (`YuvFrame::chroma_width/height` now round UP, `validate_any_extent` admits `1..=KEY_FRAME_MAX_DIM`; the GOP drivers keep the aligned `validate` contract). `KEY_FRAME_MAX_DIM` 4096 → 16384. BUG FIX en route: `apply_lr_plan` measured its SSD over the whole plane buffer — over the padded rows / columns §7.17 never touches — and tripped the elected-vs-applied debug assertion. Witness `encoder::still::tests::arbitrary_extent_stills_round_trip` (37×19 / 1×1 / 3×5 mono / 7×3 4:4:4 10-bit / 65×9 4:2:2 12-bit / 16×13, lossless and lossy, decoded through the spec driver); 256×130 / 7×3 / 1×1 stills at 8-bit 4:2:0, 4:4:4 and 10-bit decode byte-identical through TWO independent black-box reference decoders, and wrapped into AVIF by a black-box muxer they are read by four independent readers

- §5.5.2 COLOUR DESCRIPTION signalling (r460): `StillOptions::color_description` / `KeyExtras::color_description` / encoder options `color_primaries` + `transfer_characteristics` + `matrix_coefficients` code the H.273 triple with `color_description_present_flag = 1` (unspecified otherwise); with `full_range` this is what a HEIF converter reads back from the bitstream

- FRAMEWORK ENCODER (r460): `registry::make_encoder` (registered under `CodecCapabilities::with_encode`) — planar YUV / gray frames at every §6.4.1 pairing (`Yuv420P` / `Yuv422P` / `Yuv444P`, `YuvJ*`, `Gray8`, `*P10Le` / `*P12Le`, `Gray10Le` / `Gray12Le`) → one temporal unit per packet. `CodecOptions` (`Av1EncoderOptions`): `still` (reduced-header still per frame; default `false` = KEY frames under a full sequence header), `q` / `base_q_idx`, `quality` (0..=100), `lossless`, `speed` (`fast` / `balanced` / `thorough`), `tile_cols_log2` / `tile_rows_log2`, `full_range` (defaults from the pixel format). `output_params().extradata` = the `av1C` record bytes. Witnesses in `registry::tests` (still packets byte-identical to the direct `encode_still_yuv` output and decoded back through the framework decoder fed the extradata; all-intra packets; option / geometry validation)

- STILL PICTURES (r460): `encoder::encode_still_yuv{420,}(&input, &StillOptions)` codes one intra frame under a §6.4.1 `still_picture = 1` + `reduced_still_picture_header = 1` sequence header — the AVIF / HEIF `av01` image-item shape — at every §6.4.1 (bit depth, chroma format) pairing incl. monochrome, with the KEY-frame encoder's quality controls (`base_q_idx`, 0 = lossless; `StillOptions::from_quality(0..=100)`; the elected §5.9.17 delta-q plan and §5.9.12 QM ladder under `StillSpeed::Balanced`, off under `Fast`, plus the §5.9.8 superres election under `Thorough`), uniform tile layouts, `full_range` signalling and a `reduced_header = false` full-header arm. `KeyExtras::still` sets every §5.5.1 / §5.9.2 reduced-arm inference on the built sequence header (`apply_still_picture_shape`) and derives `disable_frame_end_update_cdf = 1`; operating point 0 carries the Annex A level elected from the picture size (`elect_seq_level_idx` — `MaxPicSize` / `MaxHSize` / `MaxVSize` only, a still meets no rate term; 6.0 for a 12 MP picture). `EncodedStill` carries the temporal unit, IVF wrapper, reconstruction planes, header descriptors and the `av1C` fields. Witnesses in `encoder::still::tests` (every pairing decodes to the reconstruction through the spec driver; lossless reproduces the input; tiled + full-header + option round trips)

- `codec_config::Av1CodecConfig` (r460): the `av1C` / `AV1CodecConfigurationRecord` FIELDS a container needs (`seq_profile`, `seq_level_idx`, `seq_tier`, `high_bitdepth`, `twelve_bit`, `monochrome`, `chroma_subsampling_x/y`, `chroma_sample_position`, `initial_presentation_delay_minus_one`, `config_obus`) with `from_sequence_header`, `to_bytes` / `parse` (the 4-byte record + `configOBUs`, cross-checked black-box against third-party AVIF producers' records) and `matches_sequence_header` (the av1-avif §2.2.1 "shall match" check). Bitstream-side only — no ISOBMFF boxes

- FRAMEWORK DECODER: `CodecParameters::extradata` honoured (r460). `make_decoder` feeds the configuration OBUs an `av1C` record's `configOBUs` tail (or a bare Sequence Header / temporal-delimiter-led OBU sequence) carries to the session before the first packet, so an `av01` item / ISOBMFF sample whose payload omits the sequence header decodes; payloads that repeat it, and payloads with or without a leading temporal delimiter, decode alike. Malformed configuration OBUs fail construction with `Error::invalid`. Witness `registry::tests::av1c_extradata_config_obus_seed_the_sequence_header`

- SEARCH-SIDE RATE TWIN: block-scoped pricing (r460). The r421 rate twin priced every candidate transform unit / leaf by deep-cloning the whole-frame syntax mirror (`PartitionSyntaxWriter` — ~45 per-mi grids) plus the CDF tables, and `build_search_tree` cloned the twin once per partition arm: O(frame) per priced candidate, quadratic over a picture (a 640×480 KEY frame at `q = 80` spent two thirds of its 85 s wall clock in `PartitionSyntaxWriter::clone`; a 12 MP still would have taken over an hour). The twin now keeps ONE live state behind a `RefCell`, and every `price_*` entry trial-writes onto it and rolls the block's scope back through the new `PartitionWalker::snapshot_price_scope` / `restore_price_scope` (the r431 stamp-rect snapshot generalised to `w4 × h4`, plus `TxTypes[]`, the §5.11.56 64×64-aligned `cdef_idx` anchor cells, the §5.11.39 above / left level + DC context spans of every plane widened by one mi for the sub-8×8 chroma stitch, the §5.11.9 segment-prediction context spans and the §5.11.2 `CurrentQIndex` / `DeltaLF[]` / `ReadDeltas` scalars) — O(block) per candidate plus one 15.7 KB CDF-table copy. `TuFork` became a scope on its twin (its running TU commits roll back on drop), `build_search_tree` branches through `RateTwin::scope` / `restore` (`TwinScope`: writer scope + CDFs + range) instead of cloning, and the pre-r460 deep `Clone` stays available for the inter drivers. Bit-identical output on every KEY stream (640×480 `q = 80` 84.5 s → 25.6 s, `q = 200` 61.2 s → 21.2 s); the lib suite and the KEY / format-matrix / screen-content / pixel-round-trip integration suites are unchanged
...
- SPATIAL × TEMPORAL SCALABILITY (r456): `encode_spatial_temporal_layered_gop_yuv{420,}_with_q` composes the r430 temporal ladder with the r431 spatial layers — independently coded spatial layers each carrying the dyadic `T`-deep ladder (every §7.5 unit homogeneous in `temporal_id`), §7.20 slots partitioned `8 / S` per spatial layer (temporal layer `t < T - 1` refreshes the layer's slot `t`, top-layer frames non-reference, LAST-only prediction from the own layer's frame at `i - (1 << (T - 1 - t))`, §8.3.1 primary chain on the same slot), and the `S × T` product of §6.7.5 operating points (`operating_point_idc = (spatial mask << 8) | temporal mask`) so any spatial / temporal suffix drops cleanly. The spatial driver is the same core at `T = 1` (bit-identical); `SpatialLayeredGop{,Yuv}` grow `temporal_ids` + `operating_points`. Witnesses in `tests/spatial_temporal_svc.rs` (2×2 / 2×3 / 3×2 ladders decoded at EVERY point against the per-layer reconstructions, extension-header + idc audit, slot-budget rejects); pins `self-svc-st-64-128-q84-t2` (4 points) and `self-svc-st-64-128-q72-t3` (6 points; corpus 159 → 161), every point byte-identical through TWO independent black-box reference decoders at that point (`--oppoint N --alllayers`)

- §5.9.8 SUPERRES ON INTER FRAMES — the §7.11.3.3 scaled-reference arm (r456): the P-frame driver codes a frame at the downscaled width against references held at the UPSCALED extent (`InterFrameConfig::superres`): `is_scaled( refFrame )` (§5.11.27) fires for every reference, the search context's `FrameStore` views carry the references' own `RefUpscaledWidth` / `RefFrameHeight` so every RD-scored prediction (single, compound, OBMC, inter-intra, sub-pel refinement) runs the decoder's own §7.11.3.3 scaled sampling path, the `motion_mode` read collapses to the `use_obmc` arm (priced as such by the syntax twin), the §7.11.3.1 step-7 global warp is barred (identity §5.9.24 models), the integer-pel SSD walk reads a coded-extent resample of each reference (selection aid only), the wire codes `use_superres = 1` + `coded_denom` on the §5.9.5 implicit arm, and the reconstruction is §7.16-upscaled between CDEF and §7.17 (the LR election at the upscaled extent against the original source) so the returned §7.20 payload is exactly the decoder's. Mid-GOP election on the GOP driver under the KEY arm's regime: each probe-passing P-frame trials a bounded candidate set (KEY-elected denominator or ladder midpoint + the strongest legal ratio, all inside the §6.8.2 `2 * FrameWidth >= RefUpscaledWidth` bound and the Annex A superres tile-width rule) and the joint objective over original-extent SSE + exact temporal-unit bytes keeps the winner; arming opens the sequence gate (`KeyExtras::superres_gate`) for the whole GOP, probe-failing content stays bit-identical. Witnesses in `tests/superres_inter.rs`; pin `self-gop-128x96-q180-superres-inter` (corpus 155 → 156; 364 → 335 bytes at −0.84 dB luma PSNR, every P-frame at `SuperresDenom = 16`), byte-identical through THREE independent black-box reference decoders — the corpus's first self-encoded stream whose inter frames predict through scaled references

- SUPERRES × EXPLICIT (non-uniform) TILE LAYOUTS (r456): a §5.9.15 explicit column layout rides both §5.9.8 arms (KEY election + the inter election) remapped per candidate denominator onto the DOWNSCALED superblock grid (`remap_explicit_widths`: proportional widths, column count preserved so a primary frame's per-tile donor CDFs still line up, every non-rightmost column >= 128 luma samples per the Annex A superres rule, §5.9.15 legality re-validated at the coded width; candidates without a legal remap are filtered). BUG FIX en route: the explicit tile-info writer took the superblock FLOOR of the last column / row (`next_mi >> sb_shift`) and underflowed (`0 - 1`) on every frame whose extent is not a superblock multiple — §5.9.15 sets `MiColStarts[ TileCols ] = MiCols`, so the coded `width_in_sbs_minus_1` is the CEILING; every explicit layout on a non-superblock-multiple extent panicked before (regression `non_superblock_multiple_extents_round_trip` in `tests/non_uniform_tiles.rs`). Witness `explicit_layout_rides_the_superres_arms` (320-wide `[3, 2]`: `[2, 1]` at 160, `[2, 2]` at 256; P-frames elected at 16 / 16 / 10); pin `self-gop-320x96-q180-superres-explicit-tiles` (corpus → 157), byte-identical through THREE independent black-box reference decoders — the corpus's first non-uniform-layout stream carrying `use_superres = 1` frames

- FILM GRAIN × FEATURE-EXTRA / LOSSLESS-SEGMENT TABLES (r456): the two segmentation gates the r450 §5.9.30 election kept are lifted — feature-extra tables (SEG_LVL_SKIP / SEG_LVL_GLOBALMV / SEG_LVL_REF_FRAME; the grain arm re-runs the r426 twin-priced feature trials over the denoised frames under the same table) and SEG_LVL_ALT_Q tables carrying a LOSSLESS segment (a `-base_q_idx`-clamped entry is a quantiser choice, not a pixel contract: the pre-grain §7.20 reference stays WHT-exact and §7.18.3 synthesis is output-only). Exactness-demand regions and the auto-lossless election stay out. Witnesses in `tests/film_grain_seg_extras.rs` (SKIP, GLOBALMV + REF_FRAME and `[0, -60]` tables elect on noisy split content — every P header carries BOTH the §5.9.14 table and an `apply_grain = 1` block — and decode bit-exact; clean content bit-identical to the off arm); pins `self-gop-128x96-q60-seg-skip-film-grain` and `self-gop-128x96-q60-lossless-seg-film-grain` (corpus → 159), byte-identical through THREE independent black-box reference decoders — the corpus's first feature-table × grain and lossless-segment × grain streams
...
- §5.9.2 `frame_refs_short_signaling` on the B-PYRAMID / ADAPTIVE drivers (r453): `PyramidTuning::short_ref_signaling` (default on) carries the r452 §7.8 lockstep into the out-of-order refresh graph — a coded role whose `set_frame_refs()` derivation over the TRUE stored `RefOrderHint[]` state still names every slot its RD ladder searches adopts the derived map, re-addresses its search ordinals / primary-reference candidates / §6.8.14 donor settlement onto the ordinals the derivation gave those slots (the settlement now resolves the consumed slot through the CODED map), and codes only `last_frame_idx` / `gold_frame_idx` (7 wire bits replace the explicit 22) — decoded-not-shown ALT / MID frames included; on the pyramid's role geometry the derivation lands the backward chain exactly, so every non-KEY frame rides the short arm. Witnesses in `tests/pyramid_short_refs.rs` (both drivers round-trip byte-exact, wire audit against tracked `RefOrderHint[]` state incl. the §7.8 conformance bounds, tile + donor composition, off arm keeps the explicit shape); pin `self-pyr-96x80-q60-shortrefs` (corpus 153 → 154), byte-identical through THREE independent black-box reference decoders — the corpus's first short-signaled out-of-order stream

- §5.9.30 FILM GRAIN on the B-PYRAMID / ADAPTIVE drivers (r452): the GOP-scoped election is factored into a driver-agnostic ladder (`elect_film_grain` — probe, candidate sets, perceptually-neutral-rate settlement; the GOP driver's outputs are bit-identical) and `PyramidTuning::film_grain` (default on) rides it: the arm re-runs the WHOLE out-of-order refresh graph over the denoised frames with full §5.9.30 parameters on every coded header — KEY, ALT, MID and B alike, each seeded at its own `order_hint`, so a decoded-not-shown frame's later `show_existing_frame` output carries the §7.21 `load_grain_params()` synthesis of THAT frame; the adaptive driver's cut / mini-GOP election re-runs over the denoised content inside the arm. `TunedPyramidGop*` / `TunedAdaptiveGop*` grow `film_grain_elected`. Witnesses in `tests/film_grain_pyramid.rs` (both drivers elect on noisy content and decode bit-exact on every display position; clean content stays bit-identical to the off tuning). Pin `self-pyr-96x80-q40-film-grain` (corpus 152 → 153; 16 201 → 1 885 bytes at a better neutrality score), byte-identical through THREE independent black-box reference decoders — the corpus's first grained out-of-order stream

- MEASURED, NOT ADOPTED (r452): the §5.9.30 `ar_coeff_lag = 3` ring on the election ladder. Offered as a fourth fitted candidate it never won — on white grain the 24-luma-tap header costs 2 065 B against the lag-1 ring's 1 885 B for a marginal neutrality gain (score 835 M vs 818 M), and even on noise correlated ONLY at a distance of three columns (ρ(1) = ρ(2) = 0, ρ(3) = ½, inside the probe's whiteness bound) the lag-3 candidate scored 2 083 M against lag-1's 2 076 M: the objective's correlation-match term reaches lag 1 only, so the deeper ring buys nothing it is scored on. The candidate is left off the ladder (each one is a full re-encode); the ladder stays lag 0 / 1 / 2 + csfl twins

- §5.9.2 `frame_refs_short_signaling` WRITE twin (r452): `GopTuning::short_ref_signaling` codes plain P-frames with only `last_frame_idx` / `gold_frame_idx` on the wire (7 bits replace the explicit 22) and the encoder ADOPTS the §7.8 `set_frame_refs()` derivation as its reference map — LAST / GOLDEN stay on the rotation slots (§7.8 seeds them from the explicit indices), the unsearched ordinals move to the derived slots, and every downstream twin (§7.8 sign bias, §5.9.22 skip mode, §7.9 projection) runs over the adopted map. Found the hard way: the §5.9.22 skip-mode twin resolves its second forward reference by ORDINAL order over equal hints, so a derived ordinal can name an out-of-rotation slot — those slots now carry the all-refresh FLOOR reconstruction (exactly the decoder's §7.20 store). SWITCH / coded-error-resilient positions keep the explicit shape. Witnesses in `tests/short_ref_signaling.rs` (lossy + lossless round trips, wire audit against tracked `RefOrderHint[]` state, S-frame + tile composition, off == default bit-identical); pin `self-gop-96x64-q80-shortrefs` (corpus 151 → 152), byte-identical through THREE independent black-box reference decoders

- FUZZ OOM (r452, 2026-08-29 scheduled run): the `decode` target hit libFuzzer's 2 GiB RSS ceiling. Root cause was memory AMPLIFICATION, not a live leak — the §5.11.40 `compute_tx_type()` neighbour lookup cloned the whole `MiRows × MiCols` `TxTypes[]` grid once PER TRANSFORM BLOCK, so a 76-byte still picture (2572×281) requested ~2.07 GB of transient copies and a 92-byte sibling (1024×8065) ~267 GB (28 s per exec under the sanitizer); the sanitizer's quarantine turned that churn into runner RSS. The lookup now reads the grid through a shared borrow (the 92-byte input drops 3.6 s → 0.18 s, 267 GB → under 1 GB requested). Regression inputs pinned in `tests/fuzz_regressions.rs` under a byte-counting allocator

- Annex A picture-size ceiling (r452): every parsed frame header is now gated on `UpscaledWidth * FrameHeight` BEFORE any frame-sized reservation — `decoder::MAX_PICTURE_SIZE` (35 651 584 samples, the largest defined `MaxPicSize`, levels 6.0–6.3) by default, per the Annex A level-31 note that leaves the bound to the decoder maker; `SpecDecodeSession::set_max_picture_size` lowers it for memory-constrained hosts, the new `SpecDecodeSession::decode_ivf` runs the one-shot IVF walk on a configured session, and `decode_tile_list` applies the same ceiling to its tile-count-derived output frame. Rejections surface the new `Error::PictureSizeExceedsLimit`. The `decode` fuzz target now decodes through a session capped at 2^20 samples so the harness measures panics, not the runner's RSS
...
- External tool-combination battery (r450): seven independent-encoder streams over previously untested pairings — coded error-resilient inter chains, frame-parallel CDF freezing, film-grain × reference scaling, monochrome inter, resize × superres composition (two downscales / three extents on one frame, inside the 2× conformance bound), resize × multi-tile, and 4:4:4 10-bit inter — every one decoding byte-exact on the first sweep (zero decoder fixes needed) and pinned as `ext-*` corpus entries (corpus 142 → 149). New env-gated `tests/external_sweep.rs` harness (decodes `.ivf`/`.yuv` pairs through the public entry, localizes the first diverging frame/plane) documents the acquisition workflow

- §5.11.1 EXPLICIT tile spans (r450): `GopTuning::tile_spans` emits the previously read-only `tile_start_and_end_present_flag = 1` shape on single-group multi-tile frames — `tg_start = 0 ..= tg_end = NumTiles − 1` coded in the group prologue, packaged as `OBU_FRAME_HEADER` + one `OBU_TILE_GROUP` (§5.10 bars the flag inside `OBU_FRAME`). Entropy payloads byte-identical to the flag-0 twin; single-tile and multi-group frames stay bit-identical to their baselines. Witnesses in `tests/tile_spans.rs`; pin `self-gop-128x128-q100-tilespans` (corpus 141 → 142), byte-identical through THREE independent black-box reference decoders

- CODED §5.9.2 `error_resilient_mode` on plain INTER frames (r450): `GopTuning::error_resilient_period` codes every N-th inter frame with the f(1) set — unlike a SWITCH frame the flag is a real bit and the frame keeps its INTER type + ordinary single-slot refresh, but every cross-frame decode dependency re-anchors (inferred `PRIMARY_REF_NONE` ⇒ default CDFs + `setup_past_independence`, `use_ref_frame_mvs = 0`, `allow_warped_motion = 0`, the coded `ref_order_hint[]` block over the true §7.20 slot hints, `frame_size_with_refs` bypassed). `InterFrameConfig` grows an `error_resilient` flag (implied by `s_frame`); the donor election is skipped on ER frames (no primary to elect over) while collection continues. Witnesses in `tests/error_resilient.rs` (cadence round trips lossy + lossless, coded shape off the wire, SWITCH-cadence + tiles/election composition). Pin `self-gop-96x80-q72-erm` (corpus 140 → 141), byte-identical through THREE independent black-box reference decoders — the corpus's first coded `error_resilient_mode = 1` stream

- §5.9.30 FILM-GRAIN × the §6.4.1 format axis (r450): the election's coverage walks off 8-bit 4:2:0 — MONOCHROME (the header suppresses `chroma_scaling_from_luma` and the whole chroma point/mult/offset surface; §7.18.3 synthesizes luma-only), 10/12-BIT (depth-scaled grain generation, `bit_depth − 8` LUT index split, depth-range clip in the blend), and 4:2:2 / 4:4:4 with the per-plane chroma gates UNCOUPLED (the both-or-neither constraint binds only 4:2:0), including a legal single-chroma-plane (`num_cb_points > 0, num_cr_points == 0`) header. New witnesses in `tests/film_grain_formats.rs` (mono 8/12-bit elect + clean-content rejection, 4:2:0 10-bit, 4:2:2 Cb-only, 4:4:4 12-bit — all bit-exact through the spec driver). Three new corpus pins, each byte-identical through THREE independent black-box reference decoders: `self-gop-96x80-q40-mono-film-grain`, `self-gop-96x80-q60-10bit-film-grain`, `self-gop-96x80-q60-422-cb-film-grain` (corpus 137 → 140) — the corpus's first monochrome, first >8-bit and first single-chroma-plane `apply_grain = 1` streams
...
- conformance: pin the two r447 SWITCH-frame streams (corpus 133 → 135), each digest the byte-identical output of THREE independent black-box reference decoders: `self-gop-96x64-q80-sframes` (the corpus's first self-encoded `frame_type = SWITCH_FRAME` stream — S-frames at display positions 3 and 6 refreshing all eight slots mid-GOP) and `self-splice-96x64-q60-q140-sframe` (a spliced stream whose SWITCH frame's symbols decode against reference SAMPLES coded at a DIFFERENT rate — the exact use case the tool exists for)
- conformance: pin the FOUR r444 election streams (corpus 129 → 133), each validated byte-identical through THREE independent black-box reference decoders and staged under `docs/video/av1/fixtures/`: `self-gop-128x96-q140-superres-lr` (the corpus's first stream pairing `use_superres = 1` with live §7.17 restoration — the §5.11.57 window's superres column mapping on the wire), `self-gop-128x96-q180-seg-superres` (the first segmentation × superres pairing — the segmented P chain's §5.11.19 temporal prediction resolves against the KEY's mi-mismatched `SavedSegmentIds` through the all-zero arm), `self-gop-128x96-q60-film-grain-ar` (the first self-encoded `ar_coeff_lag > 0` film-grain stream), and `self-svc-96-192-q180-superres` (the first spatially scalable stream with superres openers on BOTH §5.9.5 arms, digested at both §6.7.5 operating points; the enhancement-only output additionally cross-checked on a third decoder)
- fix(§5.9.30 conformance): the r444 chroma-points arm initially armed per plane independently; a black-box reference decoder rejected a `num_cb_points = 4, num_cr_points = 0` stream citing the §5.9.30 4:2:0 rule — film grain is applied to BOTH chroma components or NEITHER (`num_cb_points == 0` ⇔ `num_cr_points == 0` under 4:2:0 subsampling). The encoder now couples the per-plane chroma gates on 4:2:0 (both must pass, else neither arms), so that header shape can never reach the wire; 4:2:2 / 4:4:4 keep independent per-plane election per the spec note
- §5.9.30 FILM-GRAIN chroma points + AR-tap fitting (r444, part 3): the r441 election's two open §5.9.30 axes go on the wire through a CANDIDATE ladder settled by the (extended) perceptually-neutral-rate objective. Candidate 0 stays the r441 white luma-only shape (bit-identical parameters on identical input); when the residual carries structure the spec can model, further sets add (a) fitted AR taps — least squares over the §7.18.3 causal neighbourhood at lag 1 AND lag 2, each a SEPARATE candidate because every ring costs real parameter bytes on every frame header (the depth choice belongs to the score settlement, never a fit statistic; the scaling-point calibration probes re-run WITH the taps since the AR filter changes the grain template's energy), with the chroma coefficient lists carrying the same-plane taps plus the jointly-fitted LUMA-CORRELATION tap the §7.18.3 chroma filter applies at the centre position — and (b) per-plane chroma scaling points: four calibrated points at the chroma intensity-bin centres under IDENTITY §7.18.3 index mults (`cb_mult = 192, cb_luma_mult = 128, cb_offset = 256` ⇒ `merged = orig`), gated by a chroma twin of the luma probe (energy + spatial modelability + temporal decorrelation at the chroma extent — moving chroma texture never masquerades as grain). The luma probe's whiteness bound relaxes from 0.4 to 0.75 (AR-modelable correlation is now in scope; heavily correlated residual is still structure) and the objective grows chroma amplitude terms plus a luma CORRELATION-match term `samples · (ρ_src − ρ_syn)² · σ_src²` — the white candidate pays exactly for the spatial structure it cannot model. Rate-mandate refinement: the strictly-fewer-bytes demand moves PER CANDIDATE (a deeper set may outscore white on neutrality terms yet forfeit the mandate; white stays eligible on its own bytes — the r441 elections are unchanged). Witnesses (`tests/film_grain_ab.rs`): planted horizontal AR(1) noise elects `ar_coeff_lag = 1` with a decisively positive left tap and decodes bit-exact through the §7.18.3 synthesis mirror on every frame; three-plane noise elects `num_cb/cr_points = 4` with the identity mults, bit-exact; white noise keeps the r441 wire shape
- §5.9.8 SUPERRES on the spatial-SVC driver (r444, part 2): the layered shape's blocker — ONE shared sequence header must decide `enable_superres` before any layer codes — falls to a per-layer PRE-PASS election: each single-tile layer runs the standalone KEY election once (probe + arming window gate first, so out-of-regime layers spend nothing), an elected `(UpscaledWidth, SuperresDenom)` pair opens the shared gate and is FORCED onto that layer's opener under the shared header. The intra core grows the two layered §5.9.8 wire arms: a `seq_override` opener whose §5.9.5 `frame_size_override_flag = 1` fields carry the DISPLAY (upscaled) width — the override decision itself now compares the display width against the sequence maximum — with §5.9.8 re-deriving the coded width, and the `INTRA_ONLY` enhancement opener (§5.9.2 codes `frame_size()` on that arm too). Layer inter frames code their `use_superres = 0` bit under the opened gate; unelected streams stay bit-identical to the r431 shape. Witness (`tests/spatial_svc.rs`): a two-layer stream elects on BOTH openers — the base KEY on the override arm, the top `INTRA_ONLY` off the sequence maximum — and decodes bit-exact at both §6.7.5 operating points against the per-layer upscaled reconstructions
- §5.9.8 SUPERRES on the remaining drivers (r444, part 1): the r441 election's driver scope widens — (1) SEGMENTED GOPs: `GopTuning::superres` no longer requires an empty feature table; the KEY of a plain segmented GOP is itself unsegmented, and a segmented P-frame's §5.11.19 temporal prediction over the superres KEY's mi-grid-mismatched `SavedSegmentIds` takes the §5.9.2 `load_previous_segment_ids()` all-zero arm that BOTH the decode driver and the write mirror already derive through the extent-checked carry (the election simply keeps the spatial arm there); (2) the B-PYRAMID / adaptive driver (`PyramidTuning::superres`, default on): the out-of-order refresh graph reads only the KEY's UPSCALED §7.20 reconstruction and its extent-inert intra motion-field / carry seeds; (3) the §6.7.5 TEMPORAL LADDER: the multi-OP repack rewrites only the operating-point list of the KEY's own sequence header, so an elected arm's `enable_superres` gate + upscaled sequence maximum carry through and every later layer frame codes its `use_superres = 0` bit under the repacked header. Witnesses (`tests/superres_ab.rs`): all three drivers elect the arm on probe-passing content and decode bit-exact through the spec driver — the ladder additionally at a REDUCED operating point; unelected KEYs keep every driver bit-identical to the r441 shapes
- §5.9.8 × §7.17 LR-SUPERRES pairing (r444): the r441 superres election's "LR stays off the arm" scope is retired — the intra core now runs the §7.4 order exactly: after the CDEF election settles at the CODED extent, a `use_superres = 1` frame's reconstruction AND its pre-CDEF snapshot (the §7.17 stripe-boundary `UpscaledCurrFrame`) are §7.16-upscaled through the decoder's own driver, and the r429 loop-restoration election fits, prices and applies at the UPSCALED extent against the ORIGINAL (full-width) source — the §5.11.57 write window maps superblock columns through the §5.9.8 `SuperresDenom` ratio (`c · numerator / denominator`, the same derivation the reader runs), and the LR-on vs LR-off settlement stays exact realized bytes. The inner §5.9.12 / §5.9.17 elections of a superres arm now also SCORE at the upscaled extent against the original source (the r441 arms scored the coded extent; the delta-q masking weights map each upscaled column back to its coded superblock), so every stage of the arm optimises the objective the outer election settles on. Witnesses (`tests/superres_ab.rs`): a forced §5.9.8 arm on upscaler-blurred content elects Wiener units and decodes BIT-EXACT at the upscaled extent, and a plain `GopTuning::default()` GOP over horizontally-smooth / vertically-banded content ELECTS superres + self-guided LR together on the KEY, the P chain predicting from the upscaled RESTORED §7.20 reference — every frame bit-exact through the spec driver
- conformance: pin the THREE r441 election streams (corpus 126 → 129), each validated byte-identical through THREE independent black-box reference decoders and staged under `docs/video/av1/fixtures/`: `self-gop-128x96-q120-seg-qm` (the first stream pairing `segmentation_enabled = 1` with `using_qmatrix = 1` — every non-zero-segment block dequantises through its own §5.9.2 `SegQMLevel[ plane ][ segment_id ]` row), `self-gop-128x96-q180-superres` (the corpus's first self-encoded `use_superres = 1` stream — the KEY coded at the §5.9.8 denominator-16 width, §7.16 upscaled in-loop, the P chain predicting from the upscaled §7.20 reference), and `self-gop-128x96-q60-film-grain` (the corpus's first self-encoded `apply_grain = 1` stream — full §5.9.30 blocks with per-frame seeds on every header, the §7.18.3 output synthesis over a pre-grain reference chain)
- §5.9.12 multi-level QM LADDER (r441): the election grows from the single quantiser-keyed §9.5.3 level to a two-candidate ladder — the keyed level plus its LIGHTER neighbour two steps up — each a full search settled by the same exact-realized-bytes joint objective (KEY and inter drivers alike). REFINEMENT discipline per the r440 CI-budget lesson: candidates past the first run only when the keyed level already beat the flat arm (measured on the committed matrix the neighbour refines wins but never overturns a loss, so the extra full search is spent only on frames already paying for the tool); the committed matrix deltas are unchanged where the keyed level was already optimal
- §5.9.30 FILM-GRAIN election (r441): the last §5.9 parameter block with no live write arm goes on the wire — on an unsegmented lossy GOP whose content passes the noise probe (real residual energy against a 3x3-binomial denoised twin, spatially WHITE at lag 1, temporally DECORRELATED between the first two frames — static texture and clean content are rejected up front and stay bit-identical), the grain arm codes the DENOISED frames with a full §5.9.30 block on EVERY frame header (`apply_grain = 1`, `update_grain = 1` on the inter arm, per-frame `grain_seed` schedule; white grain `ar_coeff_lag = 0`, eight piecewise-linear luma scaling points at the intensity-bin centres, each CALIBRATED through the decoder's own §7.18.3 synthesis so requested amplitudes land in output units) and publishes §7.18.3-synthesized output planes while the reference chain inside the stream stays PRE-grain — exactly the decoder's §7.20 store. Elected under a documented "perceptually-neutral rate" objective (grain is output-only and never sample-matched): the grain arm's D is STRUCTURE fidelity (pre-grain recon vs the denoised source) plus a per-frame noise-AMPLITUDE mismatch penalty `luma_samples · (σ_src − σ_syn)²`, the plain arm keeps source-matched SSE, rate is exact realized IVF bytes — AND the winner must realize strictly FEWER bytes (the tool's mandate is rate; a distortion-asymmetry win alone does not elect). `GopTuning::film_grain` (default on). Measured (committed `film_grain_ab` matrix): elected wins at fine quantisers where coding the noise is expensive — amp-6 q60 −9.4 % bytes, amp-3 q60 −6.1 %, amp-3 q100 −3.1 % — with the honest plain-PSNR trade reported (~1–2 dB against the noisy source, definitional for synthetic grain); coarse-q and probe-failing configs stay bit-identical. The elected KEY + P stream decodes BIT-EXACT through the spec driver on every frame (the §7.18.3 mirror equals the decoder's grained output byte for byte)
- §5.9.8 SUPERRES election (r441): the header's last never-exercised write arm goes live — on an unsegmented lossy single-tile KEY frame inside the arming window (`base_q_idx` >= 60, luma extent >= 96x80, a HORIZONTAL-second-difference content probe: §7.16 resamples columns only, so what decides the arm is whether the horizontal spectrum survives the downscale), each candidate `SuperresDenom` (every §5.9.8 denominator whose derived coded width stays a multiple of 8) codes the frame at the DOWNSCALED width — a complete dispatcher run, the §5.9.12/§5.9.17 elections settling at the coded extent inside each arm — the reconstruction is upscaled through the decoder's OWN §7.16 driver (identical taps/subpel walk/edge clamps, so the §7.20 reference store equals the decoder's byte for byte), and the plain joint objective `D·256 + λ·R_bits256` over ORIGINAL-extent SSE + exact realized bytes keeps the winner. Wire shape: `frame_size_override_flag = 0` seeds `FrameWidth` from the sequence maximum (the UPSCALED width) and §5.9.8 re-derives the coded width from `coded_denom`; `allow_intrabc` follows its §5.9.5 `UpscaledWidth == FrameWidth` gate; LR stays off the arm (§7.17 runs at the upscaled extent — pairing open); the encoder-side DOWNSCALER ([1,2,1]/4 low-pass + centre-aligned 1/64-subpel linear resample) is an encoder choice the spec leaves open. GOP composition: a superres-elected KEY feeds the P chain its upscaled reconstruction (the §7.9 temporal scan skips intra slots on both twins, so the mi-grid mismatch is inert); `GopTuning::superres` (default on; unsegmented GOPs — a segmented P's §5.11.19 temporal prediction would read mi-mismatched `SavedSegmentIds`), pyramid / temporal-ladder / SVC drivers conservatively off (follow-ups). Measured (committed `superres_ab` matrix): on probe-passing content the arm wins across the whole band — e.g. 128x96 q100 −9.3 % bytes at +1.09 dB, q220 −12 % at −1.2 dB, 192x128 q220 −20 %; on horizontal-detail content the forced arm collapses (18 dB -> 14 dB) and the probe keeps those streams bit-identical. Hidden A/B entries `encode_key_frame_yuv420_with_q_sr{,_forced}`, `tests/superres_ab.rs` (wire audit + §5.9.8 derivation check + denominator ladder incl. a non-halving denom-9 ratio + GOP composition, all bit-exact through the spec driver)
- §5.9.12 QM × SEGMENTATION pairing (r441): the r439 quantizer-matrix election's segmented-frame gate is lifted — on an actively segmented lossy frame inside the win regime the second full search runs with the per-segment §5.9.2 `SegQMLevel[ plane ][ segment_id ]` derivation (a LOSSLESS segment takes the no-QM sentinel 15, every other segment the coded level), and the per-segment quantiser bundle (`seg_qp`) now CARRIES the frame's `using_qmatrix` + the block's own segment row through the residual chain — the neutral bundle previously dropped the QM state, so a QM-elected segmented frame would have quantised every non-zero-segment block through the flat lattice while the decoder dequantised through the §9.5.3 matrix. All SEG_LVL feature trials (ALT_Q / SKIP / GLOBALMV / REF_FRAME) price their QM-quantised chains through the same bundles; the winner still feeds the §5.9.17 delta-q election (three-way composition on the wire). New `seg_pairings` cases: election + wire audit (`segmentation_enabled = 1` with `using_qmatrix = 1` on one header) + bit-exact decode, and the lossless-sentinel composition
- conformance: pin the FOUR r439 election streams (corpus 122 → 126), each validated byte-identical through THREE independent black-box reference decoders and staged under `docs/video/av1/fixtures/`: `self-kf-128x128-q140-qm` + `self-gop-96x80-q100-qm` (the corpus's first self-encoded `using_qmatrix = 1` streams — the GOP mixes QM and flat-quantiser frames in one reference chain, 367 vs 431 baseline bytes), `self-pyr-128x64-q80-tiles-ctx-elect` (the first out-of-order stream with a patched `context_update_tile_id` — a MID frame elected tile 1 of its primary), and `self-svc-128-256-q80-ctx-elect` (the first spatially scalable stream with per-layer elected donors, digested at BOTH §6.7.5 operating points; the enhancement-only output additionally cross-checked on a third decoder)
- §5.9.12 QUANTIZER-MATRIX election (r439): the decoder's long-pinned §9.5.3 dequant tables gain their encoder side — on an UNSEGMENTED lossy frame whose luma carries real high-frequency energy (a mean-absolute-second-difference probe; smooth gradients skip the arm and stay bit-identical to the baseline), a SECOND full search runs under `using_qmatrix = 1` at a quantiser-keyed §9.5.3 level on all three planes: every quantise/dequantise step of the residual chain (KEY and inter, all TX sizes incl. the compact-`tw` 64-wide layout, palette/CfL/filter-intra residuals alike; IDTX excluded per §7.12.3) rides the QM-scaled `q2 = Round2( q * Quantizer_Matrix[..], 5 )` through the SAME `SegQMLevel[ plane ][ 0 ]` row the decoder derives per §5.9.2, and the per-TU rate twin prices the ACTUAL QM-quantised coefficient chains. Elected per frame by the plain joint objective `D·256 + λ·R_bits256` over EXACT realized (header + tile) bytes; the winner FEEDS the §5.9.17 delta-q election (the arms compose — the delta walk quantises at the running index with the same QM scaling on both sides). Default-on for every conformance-grade driver (KEY / GOP / pyramid / adaptive / temporal ladder / SVC; §7.3 camera frames conservatively stay flat), with the second full search spent only inside the MEASURED win regime — `base_q_idx` 88..=176 and luma extent >= 96x80 (fine quantisers trade bytes for distortion the flat lattice already serves; very coarse ones essentially never elect; an election-scoping choice, not a conformance constraint — it also keeps the tiered CI full-suite wall inside the hosted-runner budget); `GopTuning::qm` / `PyramidTuning::qm` A/B switches, `qm_elections` traces, `tests/qm_ab.rs` harness. Measured on the committed natural-texture matrix: up to **−14.85 % bytes at −0.067 dB** (96×80 q100 GOP) and −5.39 % at **+0.20 dB** (128×128 q140); election patterns concentrate exactly where the joint score wins, gradients bit-identical
- §6.8.14 donor election on the B-PYRAMID driver (r439): the r436 `context_update_tile_id` election extends through the out-of-order refresh graph — every coded pyramid frame (ALT / MID / B) replays its committed trees from EACH tile-donor CDF state of its ELECTED §5.9.2 primary (the r424 ordinal election may commit the nearest-BACKWARD anchor: the core donor election now follows the elected carry, main or alt, and the caller patches THAT frame's field), keeping the smallest realized §5.11.1 body. Multi-frame temporal units are handled on both patch paths: a donor frame still PENDING inside the current chunk's open unit is patched in its OBU body directly, a flushed one through the unit's frame-ordinal walk (`show_existing_frame` short headers always follow the coded frames of their unit, so ordinals are stable). Freeze-at-first-consumption per slot, swept by pointer identity (the KEY's `allFrames` refresh seeds all eight). `PyramidTuning::ctx_update_elect` (default on; inert on single-tile layouts); election trace on the tuned results
- §6.8.14 donor election on the SPATIAL-SVC driver (r439): per-LAYER donor chains on `encode_spatial_layered_gop_yuv{,420}_with_q_tiles` — each layer's KEY / `INTRA_ONLY` opener and every inter frame collect their per-tile end CDFs when THAT layer's §5.9.15 layout is multi-tile, the layer's next frame elects over them, and a win patches the donor frame at its `(temporal unit, spatial-layer frame ordinal)` wire location (§5.3.3 extension headers walk through the ordinary OBU iteration). The multi-consumer freeze discipline rides the layer-0 KEY's all-slot seeding; EVERY §6.7.5 operating point decodes the patched stream bit-exact — dropping the enhancement layer leaves each base-layer donation intact
...
- §6.8.14 `context_update_tile_id` ELECTION (r436): on multi-tile GOPs each P-frame replays its committed trees from EVERY tile of its primary frame's frame-end CDF states (§6.8.21 `load_cdfs` per candidate) and keeps the start state realizing the smallest assembled §5.11.1 body — exact realized bytes, all candidates decoding to the identical reconstruction; a win patches the PRIMARY frame's already-emitted fixed-width `f(n)` field in place (located by a two-write XOR of the header — no other bit of the stream moves) and fixes every §7.20 slot holding that frame's carry to the elected donation. `GopTuning::ctx_update_elect` (default on; `false` = the pre-r436 tile-0 shape bit for bit; inert on single-tile layouts). Measured on a designed flat/textured 2×1-tile GOP: 4 of 5 P-frames elect tile 1, every elected frame strictly smaller, 3298 → 3258 total IVF bytes (-1.2%). Corpus pin 119 (`self-gop-128x64-q80-ctx-update-elect`): the corpus's first stream with non-zero `context_update_tile_id` — THREE independent black-box reference decoders ride the patched §8.4 donations byte-identically (a decoder ignoring the field desyncs the §8.3.1 chain and fails the match)
- PER-LAYER tile layouts on the spatial-SVC driver (r436): `encode_spatial_layered_gop_yuv{,420}_with_q_tiles` codes each spatial layer under its OWN §5.9.15 uniform tile layout (`layer_tiles[s] = (TileColsLog2, TileRowsLog2)`, validated against THAT layer's §5.9.15 legality window — a 64×64 base admits only `(0, 0)` while its 256×256 enhancement takes up to `(2, 2)`) on every frame of the layer (KEY / `INTRA_ONLY` opener + inter), with §5.11.1 tile-group packaging (`tile_groups` clamps per frame to each layer's realized tile count; split frames ride `OBU_FRAME_HEADER` + `OBU_TILE_GROUP` OBUs, and the §5.3.3 extension header rides EVERY frame-carrying OBU per §7.5). `None` / all-`(0, 0)` + `tile_groups <= 1` reproduce the r431 untiled spatial stream bit for bit (pinned). Corpus pin `self-svc-tiles-128-256-q84` (corpus 118): 2×1-tiled 128×64 base under a 4×2-tiled 256×128 enhancement, both layers' frames split across two tile groups — digests byte-identical across THREE independent black-box reference decoders at BOTH §6.7.5 operating points
- conformance: pin the first multi-tile-group stream AND the first non-uniform-tile-layout stream (corpus 116 + 117) — `self-gop-192x128-q72-tiles2x2-groups2` (every frame `OBU_FRAME_HEADER` + two `tg`-sliced `OBU_TILE_GROUP` OBUs) and `self-gop-192x128-q72-tiles-nonuniform-1p2` (`uniform_tile_spacing_flag = 0`, explicit 1+2-superblock columns — a boundary the uniform walk cannot place); both digests byte-identical across THREE independent black-box reference decoders and the in-tree spec driver
- tile layouts folded into the remaining drivers (r433): `PyramidTuning::tiles` / `PyramidTuning::tile_groups` code the §5.9.15 layout + §5.11.1 tile-group packaging on EVERY frame of the B-pyramid / adaptive mini-GOP encoder (KEY, ALT, MID, B — the `show_existing_frame` chains and per-frame elections ride unchanged), and `encode_temporal_layered_gop_yuv{,420}_with_q_tiles` does the same for the §6.7.5 temporal ladder (the §5.3.3 extension header rides every split-frame OBU per §7.5, and the §5.3.1 drop rule walks the `OBU_FRAME_HEADER` + `OBU_TILE_GROUP` shape at every operating point). `(0, 0)` / `tile_groups = 1` reproduce the pre-r433 streams bit for bit (pinned); tiled pyramid (single + split groups) and the tiled 3-layer ladder decode pixel-exact through the in-tree spec driver and byte-identical across three independent black-box reference decoders
- §7.3 camera-frame tile GRIDS (r433): `encode_camera_frame_yuv420{,_tiles}` accepts heights that are multiples of 64 (up to 4096), coding one §7.3.1-conformant one-superblock-high tile ROW per superblock row (`TileRowsLog2 = ceil_log2(sbRows)` realizes uniform 1-SB rows; `W×64` stays the bit-identical single-row special case); `CameraFrameEncode::coded_tiles` carries the grid's §5.12.2 runs in tile-scan order for `(anchor_tile_row, anchor_tile_col)`-addressed tile-list entries — a 2×2 grid reassembles byte-exact through the §7.3.1 assembly and the lossless two-row arm reproduces the source exactly
- §5.9.15 NON-UNIFORM tile layouts, write side (r433): `TileInfo::explicit_layout` (the exact parser-twin derivation of the `uniform_tile_spacing_flag = 0` arm — per-column widths / per-row heights in superblock units under the `maxTileWidthSb` / recomputed-`maxTileAreaSb` / widest-column `maxTileHeightSb` bounds) + `encode_key_frame_yuv{,420}_with_q_tile_layout` and `encode_gop_yuv{,420}_with_q_tile_layout` (every frame of the GOP codes the explicit layout; `encode_gop_yuv_seg_extras_tuned_layout` is the hidden full-knob entry). The r431 `encode_tile_info` non-uniform write arm goes live end-to-end: uneven splits no uniform layout can express (1+2 / 2+1 / 1+3+1 columns, 1+2 rows) round-trip pixel-exact through the in-tree spec driver across the quantiser ladder with the CDEF/LR elections armed, illegal layouts (wrong sum, zero-size tile) reject up front, and three streams (two KEY + one 3-frame GOP) decode byte-identical across three independent black-box reference decoders
- §5.11.1 multi-tile-group FRAMES, both arms (r433). DECODE: the OBU walk now accumulates a frame's tiles across several `OBU_TILE_GROUP` OBUs (`tile_start_and_end_present_flag = 1` slices) under the §5.9.1 `SeenFrameHeader` discipline — §6.10.1 running-`TileNum` ordering enforced, the frame decodes only once `tg_end == NumTiles - 1`, `OBU_REDUNDANT_FRAME_HEADER` accepted mid-frame when its §6.8.1 `frame_header_copy` bytes are identical to the original (rejected otherwise), a mid-frame `OBU_FRAME_HEADER` rejected, an `OBU_FRAME` whose embedded group codes the flag rejected per §6.10.1, and a temporal unit ending mid-frame rejected (§7.5); new typed reject `Error::TileGroupInvalid`. ENCODE: `encode_key_frame_yuv{,420}_with_q_tile_groups` + `GopTuning::tile_groups` split every frame's tiles across N tile-group OBUs after a standalone `OBU_FRAME_HEADER` (contiguous `tg_start ..= tg_end` slices, §5.3.4 trailing bits on the header, elections and entropy payloads BYTE-IDENTICAL to the single-group stream — only the §5.2 framing changes; `tile_groups <= 1` reproduces the historical `OBU_FRAME` packing bit for bit). Validated by an in-crate repacker (every legal 4-tile grouping decodes pixel-identical, six conformance-reject shapes) and black-box: three independent reference decoders produce byte-identical output for every split shape (repacked AND encoder-native KEY g=2..4 / GOP g=2 streams) versus their single-group siblings
- TEST: the per-64×64 CDEF A/B harness (`cdef_unit_ab`) now isolates the §5.9.19 CDEF axis by holding the §5.9.17 delta-q election off (`GopTuning { delta_q: false }`), exactly as it already holds loop restoration off. The KEY delta-q arm added above is a masking-WEIGHTED `Dw + λ·R` election whose per-frame arm choice is a knife-edge on the harness's adversarial noise/edge content: a sub-percent CDEF difference on one frame propagates through the reference chain and can FLIP a later P-frame's delta-q arm (a ~200-byte swing that is entirely delta-q rate, not CDEF), turning the per-unit-vs-frame-level GOP comparison into a per-clip coin-flip even though per-unit CDEF is non-inferior in aggregate (measured pooled over six decorrelated clips: per-unit ≈ 0.2 % BETTER on the joint score). With the confound isolated the per-unit arm is a stable ~0.15 % better on the GOP joint score and strictly better on the KEY frame; the 0.5 % band still trips on a genuine per-unit CDEF regression. No encoder change
- KEY-frame §5.9.17 per-superblock delta-q election (the intra twin of the r428 inter arm): on an unsegmented lossy KEY frame the complexity probe maps per-superblock source-luma variance to reduced §5.11.13 `CurrentQIndex` units, a second full search encodes under the per-superblock plan, and the frame-level election keeps whichever arm scores better under the masking-weighted `Dw·256 + λ·R_bits256` objective over EXACT realized frame bytes (`2^(-2·units)` per-superblock weighting — flats count up, texture counts down); the elected `delta_q_present = 1` KEY carries live §5.11.13 deltas on the first coded block of each superblock (full-superblock-skip short-circuit honoured). Default-on for every KEY/GOP entry (uniform content ⇒ empty plan ⇒ bit-identical to the pre-r431 baseline); `encode_key_frame_yuv420_with_q_dq` A/B hook. Pinned as corpus 115
- conformance: pin the first multi-tile stream AND the first spatially scalable stream (corpus 113 + 114) — the r431 2x2-tiled 3-frame GOP (four §5.11.1 tile payloads per frame, uneven uniform columns, per-tile §8.2/§8.3.1 resets; digest byte-identical across THREE independent reference decoders) and the two-layer spatial stream digested at BOTH §6.7.5 operating points (full interleave + base layer) against two independent reference decoders' operating-point output; fixtures staged under `docs/video/av1/fixtures/self-gop-192x128-q72-tiles-2x2/` and `docs/video/av1/fixtures/self-svc-64-128-q72-spatial-layers/`

- spatial scalability WRITE arm (`encode_spatial_layered_gop_yuv{,420}_with_q`): 2..=4 INDEPENDENTLY CODED spatial layers under ONE sequence header (top-layer dimension budget; smaller layers ride §5.9.5 `frame_size_override_flag = 1` — inter frames the §5.9.7 no-found-ref arm), §5.3.3 `spatial_id` extension headers on every frame OBU, one shown frame per layer per §7.5 temporal unit in increasing spatial order, nested §6.7.5 operating points (`((1 << (S-k)) - 1) << 8 | 1` — every sub-bitstream still opens on the layer-0 KEY); layer 0 opens with the only KEY frame, each enhancement layer with a §5.9.2 `INTRA_ONLY` frame refreshing only its own §7.20 slot pair (new write-side INTRA_ONLY support: explicit refresh mask, `error_resilient_mode = 0`), per-layer LAST-only slot rotation + §8.3.1 primary-reference CDF chains; every operating point decodes bit-exact through the in-tree driver, and black-box: two independent reference decoders reproduce the base layer at its operating point AND the full interleave (`--oppoint` / all-layers modes) byte-identically, a third the top layer
- §5.9.15 multi-tile WRITE arm, KEY frames (`encode_key_frame_yuv{,420}_with_q_tiles` + `TileInfo::uniform_layout` + `TileInfo::tile_geometry` + `PartitionSyntaxWriter::begin_tile`): explicitly configured uniform tile columns/rows — per-tile §8.2 symbol partitions from the §8.3.1 frame-start CDF state, §5.11.1 `tile_size_minus_1` fields with the minimal realized §6.8.14 `TileSizeBytes`, §5.11.2 `clear_above_context()`/availability/LR-reference scoping per tile, tile-scoped §7.11.2 `haveAbove`/`haveLeft` + §5.11.3 tile-end `sbWidth4`/`sbHeight4` in the pixel search, tile-0 §8.4 CDF donation (`context_update_tile_id = 0`), CDEF + LR elections re-emitting and re-assembling ALL tiles; `(0, 0)` reproduces the single-tile stream byte for byte (pinned); intra-block-copy election closes on multi-tile layouts (frame-scoped DV hash index); 4-config staging set decodes byte-identical through three independent reference decoders and the in-tree spec driver
- conformance: pin the first temporally scalable stream (corpus 112) — §5.3.3 extension headers + 3-entry §6.7.5 operating-point list, digested at ALL THREE operating points against two independent reference decoders' `--oppoint` output (full 8-frame decode, the 4-frame `temporal_id <= 1` subset, the 2-frame base layer); fixture staged under `docs/video/av1/fixtures/self-gop-64x64-q72-temporal-layers/`
- §7.3 large-scale-tile WRITE arm (`encode_camera_frame_yuv420` + `build_camera_seq_yuv420_8bit`): §7.3.1-constrained camera frames through the conformance-grade generic inter driver — order-hint-free sequence (new §5.9.3 zero-`OrderHintBits` distance arm; `use_ref_frame_mvs` now derives from the sequence gates on both twins), frozen-CDF frame shape (`InterFrameConfig::freeze_cdfs` — `disable_cdf_update` + `disable_frame_end_update_cdf`), LAST-only ladder, single-tile `W×64` geometry; end-to-end suite assembles 2x2 outputs from four anchors byte-identical to the encoder reconstructions (lossless arm recovers the source exactly)
- §5.12 tile-list OBU + §7.3 large-scale-tile decoding (`tile_list` module): parse/write round trip with the §6.11 conformance bounds, the §7.3.1 ordered steps (per-entry `FrameStore[ref_frame_idx[0]] = AnchorFrames[anchor_frame_idx]`, §7.3.2 decode-camera-tile over `coded_tile_data`, raster assembly, untouched uncovered tiles), the full §7.3.1 constraint-list gate, and `decode_tile_list_stream` over SH+FH+TILE_LIST OBU packaging; general decode sessions now SKIP tile-list OBUs (§7.3.1 note) instead of rejecting the stream; new typed reject `Error::TileListInvalid`
- temporally scalable GOP encoder (`encode_temporal_layered_gop_yuv{,420}_with_q`): dyadic 2..4-layer ladder in display order (zero latency, no hidden frames), §6.7.5 operating-point list on the wire (`0x100 | ((1 << (L-k)) - 1)` masks), §5.3.3 extension headers on every frame OBU, per-layer §7.20 slot policy + LAST-slot CDF carry so every layer-drop subset decodes bit-exactly; cross-checked byte-exact against two independent reference decoders at ALL operating points (three-layer stream: full + both reduced subsets)
- operating-point selection (§5.5.1 / §6.7.5) + the §5.3.1 `drop_obu()` rule: `SpecDecodeSession::set_operating_point` / `decode_av1_at_operating_point` — OBU extension headers (temporal_id / spatial_id) honoured end to end, OBUs outside the selected point's layer set dropped before payload parse; layered-stream suite cross-checked byte-exact against two independent reference decoders at BOTH the full and the reduced operating point; new typed reject `Error::OperatingPointOutOfRange` (§6.7.5 abandon arm)
...
- av1 encoder r428: **frame-level CDEF election — §5.9.19 strengths on the wire, §7.15 mirrored on the reconstruction path** (encoder-election ladder item 3) — the sequence `enable_cdef` gate opens on every conformance-grade stream: after each lossy frame's tile is committed, the reconstruction (deblock levels are 0, so it IS the §7.15 input) is filtered through the decoder's own §7.15 driver over the write mirror's committed §5.11.56 `cdef_idx[]` anchors and 8×8 skip conjunction, a bounded strength search (coarse-then-refine primary sweep × secondary set per plane pair, damping sweep on the winner — every candidate through the real kernels) scores against the SOURCE, and a winner that beats the unfiltered frame lands in the header AND in the reconstruction — the §7.20 reference store later frames motion-compensate against, so encoder recon == decoder output stays byte-exact with the filter live. `cdef_bits = 0` ⇒ zero tile bits (the §5.11.56 literal is `L(0)`) — the election is pure distortion. HARD gates: `CodedLossless` / `allow_intrabc` (spec short-circuits), exactness-demand + auto-lossless frames (§7.15 would filter the demanded blocks and break the pixel-exact contract), and segmented GOPs (the filtered-reference perturbation destabilises the r423 temporal segment-map election chain — pairing left open). New `GopTuning.cdef` / `PyramidTuning.cdef` switches (default on), `TunedGop{,Yuv}.cdef_elections` trace, `tests/cdef_ab.rs` harness: on ringing-prone edge content **+0.28 dB at −1.2% bytes** (96×80 q140; 8-config mean +0.12 dB, per-config trade reported honestly). Corpus pin `self-gop-96x80-q140-cdef` (corpus 107 → 108): the first self-encoded stream with non-zero §5.9.19 strengths — the decoder must run the full §7.15 pipeline to reproduce it; byte-identical through THREE independent black-box reference decoders. Also fixes a debug-profile stack overflow introduced by the delta-q per-arm refactor (the per-arm artifacts — `TileCdfContext`, search context, reconstruction — are now boxed instead of moved by value through closure returns).

- av1 encoder r428: **§5.9.17 per-superblock delta-q — elected adaptive quantisation** (encoder-election ladder item 2) — on unsegmented lossy inter frames a complexity probe maps per-superblock source-luma activity to an ABSOLUTE `CurrentQIndex` plan around `base_q_idx` (geometric-mean reference: flat superblocks refine, textured coarsen; `delta_q_res = 3`, reduced units in `[-2, 2]`), a second full frame search runs under the plan — per-superblock λ tracks `CurrentQIndex` (`lambda_for` on the `delta_q_present` arm; identity elsewhere), the §5.11.2 lifecycle threads the search's twin forks so exactly the first coded block of each superblock carries the §5.11.13 value (`RateTwin::deltas_pending`), full-superblock skip leaves honour the short-circuit arm (running index carries over, mirrored encoder-side) — and a frame-level election keeps the better arm by EXACT realized (header + tile) bytes under the masking-WEIGHTED joint objective (per-superblock squared error scaled `2^(-2·units)` — variance-normalising, the same weights on both arms; under unweighted SSE a fixed-λ quantiser deviation can essentially never win, by R(D) convexity). New `GopTuning.delta_q` / `PyramidTuning.delta_q` A/B switches (default on), `TunedGop{,Yuv}.delta_q_elections` trace, `tests/delta_q_ab.rs` harness: on mixed flat/texture content the elected arm lifts the banding-prone flat region **+0.72 dB at −3.0% bytes** (128×128 q100; overall plain-PSNR trade −0.19 dB — reported honestly; uniform-spread control stays bit-identical to baseline, election never fires). Corpus pin `self-gop-128x128-q100-deltaq` (corpus 106 → 107): the first stream coding `delta_q_present = 1` + live `delta_q_abs` symbols, byte-identical through THREE independent black-box reference decoders.

- av1 encoder r428: **§5.9.2 `allow_high_precision_mv` — eighth-pel motion vectors** (encoder-election ladder item 1) — every conformance-grade inter driver (P-chain GOP / pyramid / adaptive) arms the eighth-pel precision arm: the sub-pel refinement grows a ±1-in-1/8-units pass through the real §7.11.3.4 kernels (half → quarter → eighth, each candidate scored on the decoder's own prediction), the §7.10.2 candidate stacks run the §7.10.2.10 no-op precision arm, every §5.11.32 MV-difference component codes the `mv_hp` / `mv_class0_hp` cascade with exact twin-priced bits, and the §5.9.24 global-motion estimation quantizes at the armed precision. Per-frame election by EXACT realized bits: after the tile is written, the identical committed trees replay under the quarter-pel arm (self-validating — any committed vector the coarser arm cannot re-derive or code bit-exactly errors the replay out and the frame keeps eighth-pel; ties prefer quarter-pel, making the static control bit-identical to the baseline) and the smaller tile wins; frames with non-IDENTITY global models skip the replay (precision-dependent §5.9.25 grid quantization). New `GopTuning.high_precision_mv` / `PyramidTuning.high_precision_mv` A/B switches (default on), `TunedGop{,Yuv}.hp_mv_elections` election trace, and the `tests/hp_mv_ab.rs` harness: on the 18-config sweep (eighth/mixed/integer pan × 2 sizes × q60/100/160) the armed encoder measures **−1.1% bytes at +0.33 dB mean PSNR** vs the quarter-pel baseline (up to +1.25 dB on eighth-pel pans). Corpus pin `self-gop-96x80-q60-hpmv` (corpus 105 → 106): the first stream coding `allow_high_precision_mv = 1` + odd-component NEWMV vectors, byte-identical through THREE independent black-box reference decoders.

- av1 encoder r427: **the chroma/bit-depth encoder axis — every §6.4.1 (bit depth, chroma format) pairing encodes** (ladder item 7, the campaign's last) — the conformance-grade KEY / inter-GOP / pyramid / adaptive encoders stop being 8-bit-4:2:0-only: new general entries `encode_key_frame_yuv{,_with_q}`, `encode_gop_yuv{,_with_q}`, `encode_pyramid_gop_yuv_with_q`, `encode_adaptive_gop_yuv_with_q` take a `YuvFrame` (`u16` planes + `bit_depth` + `ChromaFormat`) at 8/10/12-bit × 4:2:0/4:2:2/4:4:4/monochrome, with §6.4.1 `seq_profile` election (0 = 8/10-bit 4:2:0-or-mono, 1 = 8/10-bit 4:4:4, 2 = 8/10-bit 4:2:2 or 12-bit anything) and §5.5.2 `color_config` synthesis per pairing. The whole encoder pixel pipeline moved to one `u16` sample type: `ReconState`/reference stores/motion search/global-motion prepass/dv-hash at the stream depth, `Clip1(BitDepth)` reconstruction clamps, §7.11.2.2 depth-scaled neutral fills, §7.12.2 per-depth quantiser rows via `QuantizerParams.bit_depth`, and a depth-scaled λ (`× 2^(2·(bd−8))`, 8-bit bit-identical). Format threading is spec-complete: §5.11.5 `HasChroma` general rule, §5.11.38 `get_plane_residual_size`/`get_tx_size` at real `(subsampling_x, subsampling_y)`, general §7.11.5 CfL (`(1+subY)×(1+subX)` luma sum, `t << (3−subX−subY)`), §5.11.40 chroma-inheritance lift at `(2+subX, 2+subY)`, monochrome skips every chroma pass (`NumPlanes = 1` walk). The historical `*_yuv420` entries keep their exact signatures and route through the same core (byte-identical streams, pinned by test). The §5.11.46 palette + §5.9.20 intra-block-copy elections stay 8-bit-4:2:0-scoped (encoder choice; header gates close elsewhere).

- av1 encoder r427: **§5.11.38 partition admissibility under 4:2:2** — "it is a requirement of bitstream conformance that `get_plane_residual_size( subSize, 1 )` is not equal to BLOCK_INVALID every time subSize is computed" (UV aspect ratios stay within 1:4..4:1): the KEY and P-frame RD ladders now gate every rectangular candidate (HORZ/VERT pairs, the edge `split_or_*` rect arms, T-shapes, HORZ_4/VERT_4 strips) on the chroma validity of its subSize — under 4:2:2 the tall shapes (32×64, 8×16, 4×16 strips, ...) are never elected. Surfaced by the 4:2:2 enablement: the write mirror correctly rejected a 32×64 CfL commitment whose §8.3.2 `cfl_allowed` gate the search had derived off a masked `BLOCK_INVALID` (`unwrap_or(BLOCK_4X4)`).

- av1 r427: **format-matrix conformance: 12/12 pairings, 72/72 black-box decodes byte-exact** — `tests/format_matrix_conformance.rs` runs KEY (lossless + lossy) and 3-frame inter GOP round trips through the in-tree spec driver for all twelve §6.4.1-admissible pairings (recon == decode sample-exact; lossless == input exact; emitted `seq_profile`/color-config asserted per pairing), and the 24-stream matrix dump (one KEY + one GOP per pairing) decodes byte-identical to the encoder reconstruction in THREE independent black-box reference decoders (24/24 each).

- av1 encoder r426: **SEG_LVL_SKIP / SEG_LVL_GLOBALMV / SEG_LVL_REF_FRAME write arms + elections, three streams pinned (corpus 80 → 83)** (ladder item 8) — the decoder implemented the §5.11.10/§5.11.11/§5.11.20/§5.11.23/§5.11.25 feature arms since r394 with unit tests, but no conformance stream ever pinned them (the external encoder's CLI cannot signal the features). The encoder now codes all three: `SyntaxFrameParams` carries the per-segment feature table (every writer gate derives per BLOCK from the committed segment id — the frame-level `seg_skip_active` collapse is gone on the write side AND on the decode side, where the intra-frame prefix wrongly forced `skip = 1` onto blocks of SKIP-less segments on mixed tables; the §5.11.18 inter walk was already per-block, r394), §5.9.14 `SegIdPreSkip = 1` flips segment-id coding to the pre-skip arm on every block (skip leaves keep their own coded segment — no §5.11.9 pred inheritance), and three twin-priced per-leaf trials elect the segments: a pure-derivation SKIP block (no skip bit, silent GLOBALMV/LAST, no residual — scored as bare prediction like the §5.11.10 skip-mode trial), a GLOBALMV block with silent mode/ref over the full §5.11.17 depth ladder at the segment quantiser, and a REF_FRAME re-label of single-LAST winners (identical pixels, reference cascade + `is_inter` fall silent). Streams `self-gop-96x80-q80-seg-skip`, `self-gop-96x80-q72-seg-globalmv`, `self-gop-64x64-q60-seg-refframe` decode byte-identical in three independent black-box reference decoders + the in-tree spec driver; fixture bytes + notes staged under `docs/video/av1/fixtures/`.

- av1 encoder r426: **§7.10.2.1 TRANSLATION global-motion parameter order fixed** — the spec stores the TRANSLATION model in (row, col) order (`mv[ 0 ] = gm_params[ 0 ] >> ..` with `mv[ 0 ]` the ROW component) while the ROTZOOM/AFFINE projection reads `gm_params[ 0 ]` as the COLUMN offset; the r422 estimator packed x-first for every class, so TRANSLATION GLOBALMV predictions ran on a 90°-swapped vector since r422 — conformant on both sides (header and §7.10.2.1 agreed) and invisible to every roundtrip/pin, but the arm never won an election. The fit keeps the affine x-first convention internally (codable-model scoring included) and swaps at the emission boundary; surfaced by the SEG_LVL_GLOBALMV election work, whose pan trials kept losing on a vertically-shifted prediction.

- av1 r426: **measured: pixel-exact demand regions cost a fraction of full-lossless — and on synthetic panels beat plain lossy outright** — across the committed 5-config matrix ("typing panel over moving camera", 64×64/96×80 × q60/q100/q160, IVF bytes on identical inputs): the mixed streams run **18–56 % of the full-lossless size** (575 B vs 1 482 B; 905 B vs 3 114 B; 2 901 B vs 5 202 B; 977 B vs 3 078 B; 1 700 B vs 5 220 B) while decoding the demanded panel pixel-exact on every frame. At 64×64 q60 the mixed stream (575 B) UNDERCUTS the plain-lossy encode of the same input (907 B): the qindex-0 WHT chain codes the binary panel without lossy ringing re-coding and the exact references collapse later panel blocks to skips — cross-frame value the per-leaf greedy election structurally cannot see, which is exactly why the demand mask (not the auto election) is the exactness contract. The auto-lossless election measured INERT on this matrix: ≤8-alphabet panels are already palette-exact under the r425 screen tools (free-panel SSD 0 without any demand), and on a 12-value irregular panel the twin-priced trial honestly keeps the lossy arm at q100 (λ·ΔR > D; free-panel SSD 76 990 either way). Harness: always-on undercut + panel-exactness tripwires, env-gated `OXIDEAV_AV1_MIXLL_AB_DIR` CSV + pin-stream dump in `tests/mixed_lossless.rs`.

- av1 r426: **two mixed-lossless streams pinned (corpus 78 → 80)** — `self-gop-64x64-q60-mixll-typing` (the corpus's first per-segment lossless mixing: SEG_LVL_ALT_Q `[0, -60]` clamping segment 1 to qindex 0, TX_4X4/WHT lossless leaves inside `TX_MODE_SELECT` lossy frames, segmented KEY with §5.11.8 per-block ids, §5.11.9 bit-silent skip inheritance across a mixed `LosslessArray[]`) and `self-gop-96x80-q160-mixll-bigpanel` (multi-superblock mixing at q160 with a mi-UNALIGNED 43×30 demand panel — mask clipping + 2×2-mi group dilation — and the su(1+8) `-160` feature delta). Both digests pinned to the byte-identical display-order planar output of THREE independent black-box reference decoders, the in-tree spec driver and the encoder's own reconstruction, with the demanded panel additionally asserted sample-exact against the encoder INPUT on every frame; fixture bytes + notes staged under `docs/video/av1/fixtures/`, generation twin env-gated in `tests/mixed_lossless.rs`.

- av1 encoder r426: **per-segment lossless mixing** (ladder item 6) — a §5.9.14 `SEG_LVL_ALT_Q` segment whose §7.12.2 `get_qindex` clamps to 0 now flips the full §5.9.2 `LosslessArray[]` leaf semantics for its blocks INSIDE an otherwise-lossy frame: the TX_4X4-only §5.11.34 row-major TU grid, the §7.13.2.10 WHT chain (bit-exact residual round-trip), no §5.11.15/§5.11.16 tx-size symbols and no §5.11.47 tx-type symbol, exactly as the decoder derives per block from `Lossless = LosslessArray[ segment_id ]` (the spec decode driver was already per-segment — no decode gap found). The encoder side goes per-leaf end-to-end: the P-frame residual/depth ladder, the mirror `TxSizes[]` stamps, the skip-mode admissibility guard and the intra-fallback arm all resolve `Lossless` from the leaf's OWN committed segment; the `TuFork` twin prices both sides of a mixed-segment superblock through one adaptive CDF state (anti-desync asserted on every stream). Two §5.11.9 skip-leaf inheritance corners land with it: the bit-silent `segment_id = pred` short-circuit is frame-type-agnostic (segmented KEY frames inherit it too), and a skip leaf whose pred segment flips its `Lossless` derivation reverts its tx commitment to the spec-forced default. `encode_gop_yuv420_with_q_seg` accepts qindex-0-clamping deltas (`-255..=255` su(1+8) validation; base-0 segmentation and over-255 sums still reject).

- av1 encoder r426: **exactness-demand regions + content-driven lossless election** (ladder item 6) — new public entry `encode_gop_yuv420_with_q_lossless_regions(frames, base_q_idx, regions, auto_detect)`: caller-supplied `LosslessRegion` pixel rectangles become an mi-cell demand mask (2×2-mi group dilation covers the sub-8×8 `HasChroma` group coder and 4:2:0 cositing), and every leaf overlapping the mask is FORCED onto the lossless segment on every arm — inter residual, intra fallback (the KEY leaf encoder runs with the segment's qindex-0/`Lossless` configuration and every candidate is priced carrying the committed segment id), and the skip-mode gate (admissible over demanded cells only at zero distortion). The KEY frame segments too (§5.11.8 `intra_segment_id` per block), so the demanded region decodes PIXEL-EXACT against the input on EVERY frame — asserted per-sample (luma + chroma footprint) in `tests/mixed_lossless.rs` across aligned/unaligned rects, multi-superblock frames and a three-segment lossy-delta ladder with the lossless slot at index 2. `auto_detect` additionally lets synthetic leaves (tiny luma alphabet) OUTSIDE the mask trial the lossless segment against the lossy depth ladder on twin bits + distortion (`D = 0` by WHT bit-exactness — the arm wins exactly when the ladder's distortion outprices the rate premium).

- av1 r425: **measured: screen-content coding is now 5.49× smaller than natural coding (6.97× at pixel-exact luma; r418 stood at 4.6×)** — across the committed 18-config matrix (glyph-text page / UI panel / text-over-gradient × 192×192 + 256×144-with-clipped-bottom-edge × q20/60/100, tile-payload bytes through the search + write path on identical inputs): screen tools ON vs OFF totals 44 940 B vs 246 718 B (**5.49×**), with the 11 pixel-exact-luma configs at **6.97×** and the glyph/UI kinds quantiser-independent (identical screen bytes at q20/60/100 — fully palette/copy-coded). The r425 hash-match DV index is worth **−34.9%** on the repeated-glyph page vs the same tools with the geometric-only search (2 034 B vs 3 126 B at 192×192, every q), and never loses across the matrix. Harness: always-on pixel-exact multiple tripwire + env-gated `OXIDEAV_AV1_SCC_AB_DIR` matrix (CSV + aggregate multiples) in the `key_frame` module tests.

- av1 r425: **two streams pinned (corpus 76 → 78)** — `self-kf-256x144-q60-screen-rect` (the r425 KEY screen page: hash-seeded off-stride §5.11.7 DVs — Δrow −48 repeats the geometric set cannot reach — under the glyph-tier §5.9.20 gate, the corpus's first RECTANGULAR `PARTITION_VERT` 32×64 palette leaves and first CLIPPED `split_or_horz` 64×32 palette top blocks on the edge-straddling bottom row) and `self-gop-192x112-q60-screen-scroll` (3-frame scrolling screen page: inter text motion over clipped-leaf reconstructions, P-frame palette intra re-coding at the scroll boundaries, the first self-encoded GOP with elected — not forced — edge partitions). Both digests pinned to the byte-identical display-order planar output of THREE independent black-box reference decoders, the in-tree spec driver, and the encoder's own reconstruction; the `r425_scc_pin_page_features` witness proves the KEY tree carries all three arms; fixture bytes + notes staged under `docs/video/av1/fixtures/`, generation twin env-gated in the `key_frame` module tests.

- av1 encoder r425: **rectangular + clipped §5.11.46 palette leaves** (screen-content completion, ladder item 5) — the KEY-frame RD ladder stops being NONE/SPLIT-only: fully-in-frame `BLOCK_16X16+` nodes trial §5.11.4 `PARTITION_HORZ` / `PARTITION_VERT` with two INTRA leaves (each shape's second leaf searched against the first's committed twin state, exactly like the emitting pass), and the frame-edge half-straddle nodes elect the `split_or_horz` / `split_or_vert` single-rect arm — a clipped HORZ top / VERT left block — against the pre-r425 forced-split cascade, all under exact twin bits. The shared intra leaf encoder, the §7.11.2 mode pickers and the §5.11.46 candidate builders generalize to `(bw, bh)` rectangles, and frame-edge-clipped leaves land end-to-end: candidates build over the ACTUAL on-screen sub-rectangle, the color map carries the §5.11.49 off-screen replication fill (so §7.11.4 prediction of overhanging TUs matches the decoder bit-for-bit), the §5.11.35 walk skips off-screen-origin TUs, clip-aware residual legs zero the off-screen samples and stitch on-screen only, and the region snapshot / distortion helpers clamp to the frame. The write side's `PARTITION_HORZ` / `PARTITION_VERT` scope extends to the §5.11.4 edge arms (second block never coded on `!hasRows` / `!hasCols`). CFL stays off clipped leaves (encoder choice); the heuristic rate model keeps the pre-r425 NONE/SPLIT ladder as the A/B baseline. Witnessed: a two-strip 8+8-colour superblock elects PARTITION_VERT with palette-coded 32×64 leaves; a 64×80 dither frame elects the clipped HORZ arm (64×32 coded, 16 rows on-screen) with a palette leaf; both round-trip byte-exact through the spec driver at q60 AND lossless.

- av1 encoder r425: **intra-block-copy hash-match DV search** (screen-content completion, ladder item 5) — the §5.11.7 DV search stops depending on the r418 geometric candidate set alone: a per-frame block-hash index (`encoder::dv_hash`, own design — FNV-1a 8×8 base tier at every even position, 16/32/64 tiers composed from quadrant hashes, flat-block suppression, capped buckets) is armed whenever the §5.9.20 gate opens, and each eligible leaf probes it with its own INPUT samples to seed exact-match source candidates at ARBITRARY even offsets, nearest-first, ahead of the geometric strides. The index deliberately hashes the input plane (a reconstruction-space index only matches where the source coded losslessly); every seed still passes the full §6.10.24 `is_mv_valid` transcription, reconstruction-space SSD ranking and the exact-twin-bits RD election, so a stale seed can lose but never corrupt. The frame-level gate grows a **glyph tier**: beyond the r418 duplicate-64×64-tile scan, 16×16 grid cells (all three planes, non-flat luma) with an exact earlier duplicate at a §6.10.24-valid displacement open the gate when at least 4 cells and 1/64 of the grid match — repeated glyphs / UI patterns that never align to whole superblocks. Witnessed: a frame with per-superblock distinct backgrounds (r418 tier provably closed) and four off-stride glyph copies opens the glyph tier, commits `use_intrabc = 1` leaves at exactly the four hash-seeded DVs (q60 and lossless), and round-trips byte-exact through the spec driver's own §6.10.24 validation.

- av1 encoder r424: **deep B-pyramids — the recursive dyadic planner** — the r415 fixed two-level mini-GOP builder generalizes to a recursive dyadic pyramid of arbitrary depth (mini-GOPs up to `PyramidTuning::max_mini_gop`, default 16 = four temporal layers): the ALT anchor codes first as a decoded-not-shown reference, each interval's midpoint recurses as a further not-shown reference level, gap-2 intervals bottom out as shown non-reference B leaves, and `show_existing_frame` units display every not-shown frame at its display position. Backward reference roles come from the recursion's enclosing-anchor chain — `BWDREF` reads the nearest coded future frame, `ALTREF2` the next enclosing anchor (the §6.10 slot family's first real use), `ALTREF` the mini-GOP's ALT — with the RD ladder growing the matching singles + §5.11.25 BIDIR compound pairs ({LAST, BWDREF} / {LAST, ALTREF2} / {LAST, ALTREF}); reference rotation runs on all eight §7.20 slots via a free-list allocator (anchor + ALT + one live midpoint per level). Per-layer quantiser offsets land on the same planner (ALT finer, midpoints coarser per level, non-reference leaves coarsest; per-frame §5.9.12 header state, disabled at lossless); the r415-r423 two-level chunking remains reachable at `max_mini_gop = 4` as the A/B baseline.

- av1 encoder r424: **§5.9.2 primary-reference carry through the pyramid — exact-bytes election** — the r423 cross-frame carry (frame-end CDFs / segment ids / gm params per §7.20 slot) now flows through the pyramid's out-of-order refresh graph: every coded pyramid frame searches under its LAST-slot carry (the interval's low anchor — its most-relevant coded predecessor), then the committed trees are replayed bit-exactly under the nearest-BACKWARD anchor's carry and under the per-frame-defaults state, and the smallest total frame (header + tile — the §5.9.24 recentering against each candidate's `PrevGmParams` re-priced per candidate) wins (`InterFrameConfig::alt_primaries`). All candidates decode to the identical reconstruction — the trees carry every mode and coefficient — so the election is purely rate, per frame, and can never lose to the forced-`PRIMARY_REF_NONE` baseline by more than the 0-byte tie.

- av1 encoder r424: **adaptive mini-GOP sizing — `encode_adaptive_gop_yuv420_with_q`** — a content-driven driver over the same pyramid session: a motion-compensated MAD probe (half-resolution luma, best global integer shift within ±6 full-pel per axis) classifies each transition, scene cuts (probe > threshold) force flat P steps that absorb the discontinuity (no mini-GOP spans a cut), motion classes pick the pyramid depth (static → 16, low → 8, moderate → 4, high → 2, unpredictable → flat P), and the class boundary is settled by a twin-consistent trial-encode election — one deep chunk vs. half-depth splits over the IDENTICAL frame span, scored `D·256 + λ·8·256·bytes` with exact realized bytes (pure rate at lossless, where both reconstructions equal the input by construction).

- av1 encoder r424: **per-TU twin residue — the last proxy prices inside the residual chain are gone** (standing since r421) — a running `TuFork` (a fork of the leaf-entry rate twin: working CDFs + §6.10.2 level-context mirror + coder range) threads the leaf residual chain: every §5.11.47 tx-type candidate — on the INTER arm, the intra arm and the intrabc arm alike — now prices its ACTUAL §5.11.39 coefficient chain (the `all_zero` symbol at its true neighbour context, the `inter_tx_type` / `intra_tx_type` S() against the CURRENT adaptive CDFs at the leaf's §8.3.2 `intra_dir` axis, `eob_pt` / `coeff_base` / `coeff_br` / `dc_sign` / golomb tails) through the writer's own one-TU body (`write_single_transform_block` — the same `write_transform_block` the emitting pass runs), and the elected TU is COMMITTED into the fork so the next TU's candidates see its CDF adaptation and level-context stamps exactly as the emitting pass will. The §5.11.46 palette k-means inner ladders (1-D luma, 2-D joint-UV) stop electing `k` on the `SSE + λ·entry-cost` proxy alone: they surface their top per-`k` palettes and the exact full-leaf twin price (§5.11.46 entries + §5.11.49 tokens + the palette-predicted §5.11.39 residual chain) settles the choice. `RateModel::Heuristic` keeps the pre-r424 magnitude proxies as the measurement baseline.

- av1 r424: **measured: the twin gain moves on all three matrices** — refreshed heuristic → twin A/B on identical inputs with the per-TU residue live: 315-config intra **+0.047% bytes at +0.095 dB** (was +0.41% at +0.15 dB in r421 — the exact chain pricing pulled the byte overhead to noise while keeping the quality gain; twin smaller on 216/315), 66-config inter GOP **−3.89%** at −0.14 dB (was −3.49% in r422; smaller on 62/66), 30-config pyramid **−5.54%** at −0.25 dB (was −5.02%; the matrix now codes the r424 deep shapes on both arms; smaller on 28/30). All 411 twin streams decode byte-identical to the encoder reconstruction in THREE independent black-box reference decoders and the in-tree spec driver.

- av1 r424: **measured: deep pyramids trade rate for the ladder, the adaptive driver takes both** — across the committed 36-config A/B matrix (6 content kinds × 3 quantisers × 2 GOP lengths, 64×64 / 96×80, identical inputs): the deep tuning is worth **−4.11% bytes** at −0.15 dB against the r423 two-level baseline (smaller on 31/36), and the ADAPTIVE driver **−2.85% bytes at +0.16 dB** — smaller AND better on average, because the class election puts the depth only where it pays (static/zoom → 8/16-frame pyramids; shear/noise → flat P-runs; the boundary trial-election demotes 8 → 4+4 on the moving-texture kinds). Scene-cut configs are the adaptive win case: the probe isolates the cut behind a flat P step and takes −7.8%/−11.2%/−15.5% bytes vs the two-level baseline at q60/100/160 on the 9-frame cut matrix. The pyramid primary-reference election adopts a carried primary on essentially every frame of coherent content (8/8, 16/16) and demotes to `PRIMARY_REF_NONE` exactly at the post-cut frame (15/16 on cut GOPs — the election finding the discontinuity by rate alone). All 108 matrix streams round-trip byte-exact through the in-tree spec driver; the 72 deep/adaptive streams (and the 30 refreshed r415-matrix sweep streams) decode byte-identical in THREE independent black-box reference decoders. Harness: `tests/pyramid_deep_ab.rs` (always-on conformance A/B + behavioural witnesses — deep chunks on static content, scene-cut isolation, live primary elections; env-gated `OXIDEAV_AV1_PYR_AB_DIR` measurement matrix with CSV + stream dumps).

- av1 encoder r424: **§5.11.46 signed-delta V-plane arm election** (screen-content polish, ladder item 5 opener) — UV-palette leaves stop hard-committing the direct-literal V arm: under twin pricing each UV-palette candidate is priced under BOTH `delta_encode_palette_colors_v` arms (the delta chain and the literals decode to identical entry values, so the exact-bits smaller arm wins outright; a V list whose wrap-aware signed deltas exceed the chain's width keeps the literal arm — the writer rejects the commitment and the trial drops). Witnessed: tight V clustering (delta 6 against the 8-bit literals) provably commits `delta_encode_palette_colors_v = 1` leaves and the stream round-trips byte-exact through the spec driver's delta-arm reader; the arm is PINNED on the wire — `self-kf-64x64-q60-vdelta` (corpus 76, the first self-encoded signed-delta V-plane stream, byte-identical through the three black-box reference decoders; fixture bytes + notes staged under `docs/video/av1/fixtures/`, generation twin in `tests/screen_content_polish.rs`).

- av1 r424: **two streams pinned (corpus 73 → 75)** — `self-pyr-64x64-q60-len17-deep` (the full four-level L=16 mini-GOP: three midpoint reference levels, BWDREF / ALTREF2 / ALTREF holding three DISTINCT future frames on the deepest B leaves — the corpus's first ALTREF2 use, eight-slot §7.20 rotation, per-layer quantisers, per-frame primary-reference elections carrying a mix of LAST- and nearest-backward-carry ordinals) and `self-adaptive-96x80-q60-cut-n13` (the adaptive driver's mixed chunk lengths `[5, 1, 3, 3]` around a hard scene cut — a flat P step absorbs the discontinuity inside one coded video sequence, multi-superblock extent). Both digests pinned to the byte-identical display-order planar output of THREE independent black-box reference decoders, the in-tree spec driver, and the encoder's own reconstruction; generation is the committed env-gated `pyramid_deep_fixture_dump` twin; fixture bytes + notes staged under `docs/video/av1/fixtures/`.

- av1 encoder r423: **§5.9.2 primary-reference election — the cross-frame state carry** — P-frames now elect `primary_ref_frame = 0` (LAST) instead of `PRIMARY_REF_NONE`: the encoder carries a §7.20 per-slot store of every refreshed frame's end state — the §8.4 `save_cdfs` frame-end CDF table (the single tile's adapted state under `disable_frame_end_update_cdf = 0`), the `SavedSegmentIds` grid, and `SavedGmParams` — and each INTER frame starts from its primary slot's state per the spec loads: §6.8.21 `load_cdfs` (a clone with the per-row symbol counts zeroed), §7.21 `load_previous()` (the §5.9.24 signed-subexp coefficients now recenter against the CARRIED `PrevGmParams`, not the setup_past_independence defaults), and `load_previous_segment_ids()` (with the extent-mismatch all-zero fallback), replacing the per-frame default re-init. The §5.9.14 flags become CODED bits (`update_map = 1`, `update_data = 1`, `temporal_update` elected per frame); KEY frames deposit their own carry through the `allFrames` refresh. The rate twin snapshots the inherited state at every superblock, so the r421 anti-desync invariant (search-committed symbols ≡ writer-emitted symbols) holds unchanged across the whole GOP.

- av1 encoder r423: **§5.11.19 temporal segment-map coding + exact-bits election** — the `segmentation_temporal_update == 1` write arm is fully threaded through the driver: per block, the §5.11.21 `get_segment_id()` prediction over the carried `PrevSegmentIds`, the §8.3.2 seg-pred ctx (`LeftSegPredContext[ MiRow ] + AboveSegPredContext[ MiCol ]`, read BEFORE the block's own stamp per spec order), the binary `seg_id_predicted` S() (adopt the prediction on a hit; §5.11.20 spatial fallback on a miss), and the two spec-mandated context-array stamps (the `!preSkip && skip` forced-0 arm and the temporal arm) mirrored onto the write-state walker. The frame header's `temporal_update` is elected by EXACT realized bits: the main pass searches and emits under the SPATIAL arm (bit-identical trees to the temporal-disabled baseline), then the committed per-superblock trees replay bit-deterministically under the temporal arm from the same §8.3.1 frame-start CDF state, and the smaller tile wins — both arms decode to the same reconstruction (segment ids, modes and coefficients are the trees'), so the election is purely rate AND the elected stream is smaller-or-equal per frame than the spatial-only encode by construction (a temporal-searched first cut measurably lost to spatial-native searches on a third of the matrix; the flipped election excludes that outcome structurally).

- av1 encoder r423: **skip-leaf segment invariant in the shared intra-leaf ladder (bug fix)** — on a segmented inter frame a `skip == 1` leaf's segment id is the bit-silent §5.11.9 spatial `pred` (the §5.11.19 skip arm's short-circuit), but the shared intra-leaf ladder priced its trial candidates with the constructor-default segment 0 — any intra candidate that quantised to all-zero next to a nonzero-segment neighbour hard-errored the whole encode (latent since the r421 twin pricing put the write path's validation inside candidate pricing; surfaced by the r423 persistent-segment matrices, reproducible on the pre-r423 tree). Trial candidates now carry the twin-derived pred before pricing (`RateTwin::spatial_segment_pred` — the §5.11.9 cascade over the twin's own write-state mirror), on the ladder and on the single-shape early return alike.

- av1 r423: **measured: the carry pays, the temporal arm never loses; two streams pinned (corpus 71 → 73)** — across the committed 12-config A/B matrix (4 persistent-segment content kinds × 3 quantisers, SEG_LVL_ALT_Q `[0, 24, 48]`): the primary-reference carry (CDF inheritance + coded §5.9.14 flags) is worth **−1.52%** total bytes against the `PRIMARY_REF_NONE` baseline (smaller on 12/12 configs), and the temporal segment-map election takes a further **−0.06%** (smaller-or-equal on 12/12 — the flipped election's structural guarantee — electing `temporal_update = 1` on up to 4 of 5 P-frames per GOP, best single config −0.28%). All 36 variant streams round-trip byte-exact through the in-tree spec driver; the 12 default-tuning streams decode byte-identical in THREE independent black-box reference decoders. `self-gop-128x64-q72-seg-temporal-moving` (moving segmented region — §5.11.19 adoptions inside the region, spatial fallback on the moving edges) and `self-gop-192x128-q72-seg-temporal-static` (the win case — `temporal_update = 1` on 4/5 P-frames) are pinned with digests, fixture bytes + notes staged under `docs/video/av1/fixtures/`.

- av1 r423: **temporal-segmentation witnesses + A/B harness** — `tests/temporal_segmentation.rs`: always-on persistent-segment round trips (static and moving flat regions over scrolling texture) with an election witness (the static GOP MUST elect `temporal_update` on at least one P-frame), per-variant conformance (primary-ref off / temporal off — the switches trade rate, never correctness), the structural aggregate tripwires (temporal ≤ spatial ≤ primary-none on the persistent matrix), an unsegmented CDF-inheritance A/B, and a lossless-arm carry round trip; env-gated (`OXIDEAV_AV1_SEG_AB_DIR`) 12-config measurement matrix writing per-config bytes for the three variants (CSV + aggregate deltas) and every default-tuning IVF for external black-box decoder validation.

- av1 encoder r422: **mode-cascade candidate rates on the rate twin** — the last big INTER-path heuristic converts to exact bits: the §5.11.23 mode + MV prefix (the §5.11.25 reference cascade, the four-arm `YMode` dispatch, the `drl_mode` loop, the NEWMV `read_mv` differences) is factored into ONE writer body (`write_inter_mode_mv_prefix`) that the emitting pass runs inside `write_inter_block_mode_info` and the twin runs inside the new `RateTwin::price_inter_mode` — pricing and committing share the emission path by construction. Every leaf candidate (NEWMV per-drl-slot choice, NEARESTMV / NEARMV, GLOBALMV under the r422 elected models, the compound ladder, NEW_NEWMV) is scored `D·256 + λ·bits256` against the twin's current adaptive CDFs; the masked-compound / jnt / inter-intra side-data trials ride the exact prefix price plus their prior symbol proxies promoted to 1/256 units. Refreshed A/B matrices (heuristic → twin, same inputs): 66-config inter GOP **−3.49% bytes** at −0.07 dB (smaller on 63/66; r421's proxy-mode-rate twin stood at −3.06%), 30-config pyramid −5.02% at −0.17 dB, intra unchanged (the prefix is inter-only); the full 411-stream twin sweep decodes byte-identical in THREE independent black-box reference decoders (1233/1233 runs) with the global-motion election live.

- av1 encoder r422: **GLOBAL warped-motion election — the frame header carries real models** — a frame-level global-motion election lands on every inter frame: a coarse motion pre-pass per distinct reference (exhaustive half-resolution scan over 2×2-mean planes — fine-texture aliasing and reference coding blur wash out — then full-pel and 1/8-pel bilinear refinement, one sample per 16×16 block), least-squares fits of the three §6.8.18 model classes (TRANSLATION mean, ROTZOOM via the 4-unknown joint normal equations whose α/b cross-terms cancel, AFFINE via two independent 3-unknown systems), one outlier-trim + refit round, §5.9.25 grid quantization BEFORE scoring (the election judges the codable model), §7.11.3.6 `setup_shear` validation for the warp classes, and a residual-energy gate (a model must explain ≥ 75% of the measured motion energy on a genuinely-moving field; class upgrades must win by ratio AND absolute margin). The elected `(GmType, gm_params)` are stamped into ONE shared source of truth — the inter parameter bundle feeding the §7.10.2.1 `GlobalMvs` derivation and the §7.11.3 global-warp prediction of the search mirror, the write pass and the decoder alike — so the model can only change which streams the RD ladder prefers, never desync one.

- av1 encoder r422: **§5.9.24/§5.9.25 WRITE arm — the signed-subexp global-motion coefficients** — the full `read_global_param` inverse: the §5.9.29 `recenter` forward, the §5.9.28 bucket-ladder `encode_subexp` (`ns()` tail + `subexp_more_bits` climb), the §5.9.26/§5.9.27 signed/unsigned-with-ref wrappers (both recenter arms), the per-type §5.9.24 loop (1–3 type bits per ref; TRANSLATION / ROTZOOM / AFFINE coefficients in the reader's exact order with the derived ROTZOOM `[4]/[5]` pair), and `quantize_global_param` — the nearest-ordinal grid snapper (per-slot `absBits`/`precBits`, `allow_high_precision_mv`-aware, `[-mx, mx]` clamped) the election runs so encoder-side `gm_params` equal the decoder's reconstruction exactly. Byte-exact against the crate's own §5.9.24 parser on synthetic ordinal sweeps covering both recenter arms, every subexp bucket shape, both precision modes, the range extremes, and non-default `PrevGmParams` tables.

- av1 r422: **measured: elected global motion pays on warp content; two streams pinned (corpus 69 → 71)** — across the committed 30-config A/B matrix (identity-only → elected models on identical inputs, 5 content kinds × 2 sizes × 3 quantisers): **+0.92% bytes for +0.53 dB mean PSNR**, with the warp kinds decisive — rotation-64x64-q60 **+1.30 dB** at +6 B, zoom-64x64-q100 **+1.42 dB**, zoom/zoompan-96x96-q60 both smaller AND ~+1 dB — and the static control bit-identical (an IDENTITY election leaves the stream untouched). All 30 elected-model streams decode byte-identical to the encoder reconstruction in THREE independent black-box reference decoders and the in-tree spec driver; `self-gop-64x64-q60-gm-zoom-warp` and `self-gop-64x64-q60-gm-rotation` (non-identity ROTZOOM headers, §5.9.25 signed-subexp coefficients on the wire, global-warp prediction live) are pinned with digests, fixture bytes + notes staged under `docs/video/av1/fixtures/`.

- av1 r422: **global-motion witnesses — pan / zoom / rotation content elects and round-trips** — `tests/global_motion_conformance.rs`: on synthetic content whose motion field IS a global model, the encoder provably elects the matching class on the LAST reference of most P frames (TRANSLATION on a (+2.0, +1.25)-pel pan at the exact coded vector; ROTZOOM with the §5.9.24 derived pair on a 3.5%-per-frame zoom and a ~1.7°-per-frame rotation), static content provably stays IDENTITY, and every stream still decodes byte-exact through the spec-faithful frame driver against the encoder's own reconstruction — with the non-identity `GlobalMvs`, GLOBALMV re-pricing and ≥ 8×8 global-warp prediction live on every leaf. Env-gated (`OXIDEAV_AV1_GM_DIR`) stream dumps feed external black-box decoder validation and corpus pinning.
...
- av1 r421: **measured: exact rate costs pay for themselves; two streams pinned (corpus 67 → 69)** — across the committed A/B matrices (heuristic → twin on identical inputs, encoder's own joint `SSE + λ·bits` objective never worse): 66-config inter GOP **−3.06% bytes** at −0.05 dB mean PSNR (twin smaller on 62/66), 30-config pyramid **−4.98% bytes** at −0.19 dB (27/30), 315-config intra +0.41% bytes for **+0.15 dB** (159/315 smaller; the byte regressions pair with outsized PSNR gains). The r419 OBMC-at-q60 RD-mixed flag re-judged: on q60 shear content the twin encode is 2.9% smaller AND 0.13 dB better, and the OBMC selection witness still commits OBMC leaves under exact costs. All 411 twin-model sweep streams decode byte-identical to the encoder reconstruction in THREE independent black-box reference decoders and the in-tree spec driver; `self-gop-64x64-q60-shear-ratetwin` (the re-judged configuration) and `self-pyr-176x144-q255-shear-ratetwin` (the sweep's largest saving, −27%) are pinned with digests, fixture bytes + notes staged under `docs/video/av1/fixtures/`.
